# 📋 DOCUMENT D'ÉVALUATION COMPLET — INCEPTION OF THINGS
## Préparation Soutenance 42 — Niveau Évaluateur Expérimenté

---

> **Mode d'emploi de ce document**
>
> Ce document est conçu pour simuler une évaluation 42 rigoureuse. Il est structuré en deux blocs :
> 1. **Exercices** (Parties 1 à 8) — sans aucune réponse, pour un travail à blanc honnête
> 2. **Corrigé complet** — à la fin du document, avec explications détaillées de pourquoi chaque réponse est correcte ou fausse
>
> **Conseil d'utilisation** : traite chaque partie intégralement avant de consulter le corrigé correspondant. Note tes réponses sur une feuille séparée. Un évaluateur 42 sincère ne regarde jamais la correction avant d'avoir tranché.

---

# TABLE DES MATIÈRES

- **Partie 1** — QCM (120 questions)
- **Partie 2** — Questions théoriques ouvertes (100 questions)
- **Partie 3** — Analyse de code Shell (30 exercices)
- **Partie 4** — Analyse de Vagrantfile / Ruby (30 exercices)
- **Partie 5** — Réseau (50 exercices)
- **Partie 6** — Kubernetes / K3s (50 exercices)
- **Partie 7** — Debugging (50 scénarios)
- **Partie 8** — Oral blanc 42 (100 questions)
- **CORRIGÉ COMPLET** — Toutes les réponses et explications
...
> Pour chaque question, une ou plusieurs réponses peuvent être correctes. Identifie-les toutes.

## Section 1.A — Vagrant (questions 1-20)

**Q1.** Que fait la commande `vagrant up` lors de la toute première exécution dans un dossier ?
A) Elle démarre uniquement une VM déjà existante
B) Elle télécharge la box si elle n'est pas en cache local, puis crée et démarre la VM
C) Elle compile VirtualBox depuis les sources
D) Elle nécessite obligatoirement une connexion à Vagrant Cloud à chaque exécution

**Q2.** Le dossier `.vagrant/` créé après un `vagrant up` contient :
A) Le code source de la box
B) Les informations d'état de la VM (ID, provider utilisé, clé SSH privée générée)
C) Le Vagrantfile compilé en binaire
D) Rien d'important, il peut être supprimé sans conséquence

**Q3.** Quelle commande permet de relancer les scripts de provisioning sans redémarrer la VM ?
A) `vagrant reload`
B) `vagrant provision`
C) `vagrant up --provision`
D) `vagrant halt && vagrant up`

**Q4.** Avec `config.vm.box_check_update = false`, que se passe-t-il ?
A) Vagrant n'installe jamais de box
B) Vagrant ne contacte pas Vagrant Cloud pour vérifier une nouvelle version de la box à chaque `vagrant up`
C) Les mises à jour de sécurité du système ne sont jamais installées
D) Cela désactive le réseau de la VM

**Q5.** La commande `vagrant destroy -f` :
A) Supprime uniquement les fichiers de configuration
B) Supprime la VM et son disque virtuel définitivement, sans confirmation
C) Met la VM en pause
D) Supprime aussi la box téléchargée en cache

**Q6.** Que représente le paramètre `"2"` dans `Vagrant.configure("2") do |config|` ?
A) Le nombre de VMs à créer
B) La version de l'API Vagrant utilisée par le fichier de configuration
C) Le nombre de CPU alloués
D) La version de Ruby requise

**Q7.** Quel fichier Vagrant lit-il par défaut dans le répertoire courant ?
A) `vagrant.yaml`
B) `Vagrantfile`
C) `vagrant.rb`
D) `config.vagrant`

**Q8.** À quoi sert `vagrant ssh-config` ?
A) Modifier la configuration SSH du serveur
B) Afficher les paramètres de connexion SSH (port, clé privée, host) utilisables par un client SSH externe
C) Désactiver SSH sur la VM
D) Générer une nouvelle paire de clés SSH

**Q9.** Quelle est la différence entre `vagrant halt` et `vagrant suspend` ?
A) Aucune différence
B) `halt` éteint proprement l'OS (comme un shutdown), `suspend` sauvegarde l'état RAM sur disque
C) `halt` supprime la VM, `suspend` la garde
D) `suspend` nécessite une box différente

**Q10.** Un Vagrantfile peut définir plusieurs VMs grâce à :
A) Plusieurs fichiers Vagrantfile dans le même dossier
B) Des appels multiples à `config.vm.define "nom" do |var| ... end`
C) La commande `vagrant multi-create`
D) Ce n'est pas possible, une seule VM par Vagrantfile

**Q11.** Le plugin `vagrant-vbguest` sert à :
A) Ajouter un utilisateur invité sur la VM
B) Détecter et corriger les décalages de version entre VirtualBox et les Guest Additions de la box
C) Activer le réseau bridge automatiquement
D) Chiffrer le disque de la VM

**Q12.** Que se passe-t-il si deux VMs définies dans le même Vagrantfile utilisent la même IP en `private_network` ?
A) Vagrant fusionne automatiquement les deux VMs
B) Cela peut provoquer des conflits réseau ou des comportements imprévisibles
C) Vagrant refuse de démarrer et lève une erreur de syntaxe immédiate
D) Aucun souci, chaque VM est isolée

**Q13.** La directive `config.vm.box_check_update` agit sur :
A) La vérification de mise à jour du Vagrantfile
B) La vérification de mise à jour de la box utilisée comme image de base
C) La vérification de mise à jour de VirtualBox
D) La vérification de mise à jour de K3s

**Q14.** `vagrant global-status` affiche :
A) Uniquement les VMs du dossier courant
B) Toutes les VMs Vagrant connues sur la machine, peu importe le dossier
C) L'état réseau de toutes les VMs
D) Les logs de provisioning

**Q15.** Pourquoi un script shell de provisioning peut-il échouer silencieusement sans `set -e` ?
A) `set -e` n'a aucun effet sur les scripts de provisioning
B) Sans `set -e`, le script continue après une commande en erreur, masquant potentiellement un échec critique
C) Vagrant ignore systématiquement les erreurs de script
D) C'est impossible, Vagrant stoppe toujours sur erreur

**Q16.** L'option `--provider` de `vagrant up` sert à :
A) Choisir le fournisseur d'accès internet de la VM
B) Forcer l'utilisation d'un hyperviseur particulier (virtualbox, libvirt...) si plusieurs sont installés
C) Choisir le repository Git du projet
D) Spécifier le mainteneur de la box

**Q17.** Que fait `vagrant reload --provision` ?
A) Recharge uniquement le réseau
B) Redémarre la VM ET réexécute tous les scripts de provisioning
C) Supprime puis recrée la VM
D) Met à jour Vagrant lui-même

**Q18.** Dans un Vagrantfile avec plusieurs VMs, l'ordre de définition des blocs `config.vm.define` :
A) N'a aucune influence sur l'ordre de démarrage
B) Détermine généralement l'ordre dans lequel Vagrant traite/démarre les VMs (sauf parallélisme)
C) Doit obligatoirement aller du plus petit au plus grand nom alphabétique
D) Doit être identique à l'ordre des IPs

**Q19.** Que retourne `vagrant status NOM_VM` si la VM n'a jamais été créée ?
A) Une erreur fatale qui arrête le terminal
B) L'état `not created`
C) L'état `running` par défaut
D) Il faut obligatoirement utiliser `vagrant status` sans argument

**Q20.** Le fichier `insecure_private_key` mentionné dans les logs Vagrant sert à :
A) Chiffrer le disque dur de la VM
B) Une clé SSH par défaut peu sécurisée que Vagrant remplace automatiquement par une clé générée propre à chaque VM
C) Une clé pour accéder à Vagrant Cloud
D) Une clé pour le chiffrement du réseau privé

---

## Section 1.B — VirtualBox / Libvirt / KVM / QEMU (questions 21-40)

**Q21.** VirtualBox est un hyperviseur de type :
A) Type 0
B) Type 1 (bare-metal)
C) Type 2 (hosted, tourne sur un OS hôte)
D) Type 3

**Q22.** KVM (Kernel-based Virtual Machine) est :
A) Un logiciel utilisateur indépendant du noyau Linux
B) Un module du noyau Linux qui transforme Linux en hyperviseur de type 1
C) Une marque déposée de VirtualBox
D) Un format de fichier disque virtuel

**Q23.** QEMU dans le contexte de KVM/libvirt sert principalement à :
A) Remplacer entièrement KVM
B) Émuler les composants matériels (disque, réseau, carte graphique...) pendant que KVM gère l'accélération CPU
C) Gérer uniquement le réseau de la VM
D) Compresser les images disque

**Q24.** Libvirt est :
A) Un hyperviseur en lui-même
B) Une couche d'abstraction/API qui permet de gérer différents hyperviseurs (KVM, Xen...) de façon unifiée
C) Un remplaçant direct de VirtualBox avec la même API
D) Un protocole réseau

**Q25.** La commande `virsh list --all` permet de :
A) Lister toutes les box Vagrant
B) Lister toutes les VMs gérées par libvirt, actives ou non
C) Lister les interfaces réseau du système
D) Lister les paquets installés

**Q26.** VT-x (Intel) et AMD-V (AMD) désignent :
A) Des logiciels de virtualisation propriétaires
B) Des extensions matérielles du processeur permettant la virtualisation assistée par le hardware
C) Des protocoles réseau pour VMs
D) Des formats de disque virtuel

**Q27.** Si la virtualisation matérielle (VT-x) n'est pas activée dans le BIOS, que se passe-t-il généralement avec VirtualBox ?
A) VirtualBox fonctionne normalement sans aucune limite
B) VirtualBox peut refuser de démarrer la VM ou afficher une erreur de type VERR_VMX_NO_VMX
C) Cela n'affecte que le réseau
D) Cela accélère le démarrage des VMs

**Q28.** Un fichier `.vdi` est associé à :
A) Un format de disque virtuel propre à VirtualBox
B) Un format de configuration réseau
C) Un fichier de log Vagrant
D) Un format de box Vagrant

**Q29.** Si `vagrant status` affiche `running (virtualbox)` pour une VM, mais que `virsh list --all` ne montre rien, cela signifie :
A) Une erreur de configuration obligatoire à corriger
B) C'est normal : la VM est gérée par VirtualBox, pas par libvirt, ce sont deux piles de virtualisation indépendantes
C) Il faut migrer la VM vers libvirt immédiatement
D) Le Vagrantfile est corrompu

**Q30.** Sur une machine où VirtualBox ET libvirt sont installés, comment forcer Vagrant à utiliser VirtualBox ?
A) C'est impossible si libvirt est présent
B) Avec `vagrant up --provider=virtualbox` ou la variable d'environnement `VAGRANT_DEFAULT_PROVIDER`
C) En désinstallant libvirt obligatoirement
D) En modifiant le noyau Linux

**Q31.** Le "Storage pool" mentionné dans les logs libvirt désigne :
A) Une zone de stockage réseau partagé entre VMs
B) Un emplacement où libvirt stocke les disques virtuels des VMs (souvent `/var/lib/libvirt/images/`)
C) Un cache mémoire du CPU
D) Un protocole de chiffrement

**Q32.** VBoxManage (ou vboxmanage) est :
A) L'interface graphique exclusive de VirtualBox
B) L'outil en ligne de commande pour administrer VirtualBox sans passer par l'interface graphique
C) Un outil de Vagrant uniquement
D) Un outil propre à libvirt

**Q33.** Les VirtualBox Guest Additions servent notamment à :
A) Augmenter artificiellement le nombre de CPU virtuels
B) Permettre les dossiers partagés, une meilleure intégration graphique, et la synchronisation d'horloge
C) Remplacer le noyau Linux de la VM
D) Gérer le pare-feu de l'hôte

**Q34.** Si la version des Guest Additions (ex: 7.0.2) ne correspond pas à la version de VirtualBox installée (ex: 7.2), quel risque concret est mentionné dans les avertissements Vagrant ?
A) La VM refuse systématiquement de démarrer
B) Les dossiers partagés (synced folders) peuvent ne pas fonctionner correctement
C) Le CPU virtuel est automatiquement bridé
D) Le réseau privé est désactivé automatiquement

**Q35.** `vboxsf` est :
A) Un protocole de chiffrement réseau VirtualBox
B) Le système de fichiers utilisé par VirtualBox pour implémenter les dossiers partagés
C) Un format d'image disque
D) Le nom du processus principal de VirtualBox

**Q36.** Pourquoi un cluster K3s en VM peut-il être plus performant sous KVM/libvirt que sous VirtualBox sur Linux ?
A) Ce n'est jamais le cas, VirtualBox est toujours plus rapide
B) KVM utilise la virtualisation matérielle native du noyau Linux avec généralement moins d'overhead que VirtualBox
C) Libvirt n'a pas besoin de CPU
D) VirtualBox ne supporte pas Linux comme invité

**Q37.** Dans les logs `Graphics Type: vnc`, cela signifie typiquement :
A) Le réseau de la VM utilise VNC
B) L'accès à l'affichage graphique de la VM se fait via le protocole VNC, courant avec libvirt
C) Le disque de la VM est de type VNC
D) C'est une erreur de configuration critique

**Q38.** Quelle affirmation décrit le mieux la relation entre QEMU et KVM ?
A) QEMU remplace totalement KVM dans tous les cas
B) QEMU peut fonctionner seul (émulation pure logicielle, lent) ou s'appuyer sur KVM pour accélérer l'exécution via le matériel
C) KVM ne peut jamais fonctionner sans QEMU
D) Les deux sont strictement incompatibles

**Q39.** Quelle commande permet de vérifier la version de VirtualBox installée sur l'hôte ?
A) `vboxmanage --version`
B) `virtualbox --check`
C) `vagrant --vbox-version`
D) `VBoxService --hostversion`

**Q40.** L'image `.img` mentionnée dans les chemins libvirt (`/var/lib/libvirt/images/...img`) représente :
A) Une capture d'écran de la VM
B) Le disque virtuel de la machine (équivalent du .vdi pour VirtualBox)
C) Un fichier ISO d'installation
D) Un fichier de log

---

## Section 1.C — Réseau (questions 41-65)

**Q41.** Une adresse IP de la forme `192.168.56.110` appartient à :
A) Une plage publique routable sur Internet
B) Une plage d'adresses privées (RFC 1918)
C) Une adresse de loopback
D) Une adresse de multicast

**Q42.** Le masque `/24` en notation CIDR correspond à :
A) `255.255.0.0`
B) `255.255.255.0`
C) `255.0.0.0`
D) `255.255.255.255`

**Q43.** Avec un réseau `192.168.56.0/24`, combien d'adresses IP utilisables pour des hôtes sont théoriquement disponibles (en excluant adresse réseau et broadcast) ?
A) 256
B) 254
C) 128
D) 512

**Q44.** Dans Vagrant, le réseau `"private_network"` avec une IP fixe :
A) Rend la VM accessible directement depuis Internet
B) Crée un réseau host-only où l'hôte et les VMs peuvent communiquer entre eux, isolé d'Internet
C) Désactive complètement le réseau de la VM
D) Nécessite obligatoirement un serveur DHCP externe

**Q45.** Le réseau NAT par défaut de Vagrant sert principalement à :
A) Permettre à la VM d'accéder à Internet via l'hôte
B) Permettre à l'hôte d'accéder directement aux services de la VM sans configuration
C) Isoler complètement la VM du réseau
D) Servir de réseau pour la communication inter-VMs uniquement

**Q46.** Quelle est la différence fondamentale entre un réseau "Host-only" et un réseau "Bridge" ?
A) Aucune différence
B) Host-only isole les VMs et l'hôte du réseau physique externe ; Bridge connecte la VM directement au réseau physique comme une machine à part entière
C) Bridge est toujours plus lent que Host-only
D) Host-only nécessite obligatoirement une carte réseau physique dédiée

**Q47.** Un "Gateway" (passerelle) réseau sert à :
A) Filtrer uniquement les paquets UDP
B) Router le trafic d'un réseau local vers d'autres réseaux (notamment Internet)
C) Résoudre les noms de domaine
D) Chiffrer les communications HTTPS

**Q48.** Le protocole TCP, comparé à UDP :
A) N'offre aucune garantie de livraison, comme UDP
B) Est orienté connexion et garantit (sauf cas extrêmes) la livraison ordonnée des données
C) Est toujours plus rapide qu'UDP dans tous les cas
D) Ne peut pas être utilisé pour HTTP

**Q49.** Un port comme `6443` (utilisé par l'API Kubernetes) sert à :
A) Identifier une machine sur le réseau
B) Identifier un service/processus spécifique sur une machine ayant une IP donnée
C) Chiffrer la communication automatiquement
D) Remplacer le besoin d'adresse IP

**Q50.** DHCP (Dynamic Host Configuration Protocol) sert à :
A) Chiffrer le trafic réseau
B) Attribuer automatiquement une adresse IP (et d'autres paramètres réseau) à une machine qui se connecte
C) Résoudre les noms de domaine en IP
D) Router les paquets entre réseaux

**Q51.** DNS (Domain Name System) sert à :
A) Attribuer des adresses IP dynamiquement
B) Traduire des noms de domaine lisibles (ex: app1.com) en adresses IP
C) Chiffrer les communications
D) Gérer les ports ouverts d'un firewall

**Q52.** Dans le projet IoT Partie 2, l'en-tête HTTP `Host: app1.com` envoyé avec curl sert à :
A) Changer l'adresse IP de destination réellement utilisée par le paquet
B) Indiquer au serveur (Ingress/Traefik) quel virtual host est demandé, pour le routage applicatif
C) Forcer l'utilisation du protocole HTTPS
D) Définir le port de destination

**Q53.** Une "socket" réseau est définie de façon unique par :
A) L'adresse IP seule
B) La combinaison adresse IP + numéro de port (+ protocole)
C) Le nom de domaine seul
D) L'adresse MAC seule

**Q54.** Si deux VMs sont sur le même réseau `private_network` (192.168.56.0/24) mais qu'une a pour IP 192.168.56.110 et l'autre 192.168.57.50, que se passe-t-il sans route explicite ?
A) Communication parfaitement normale, aucun souci
B) Elles ne sont pas sur le même sous-réseau /24, la communication directe sans routage peut échouer
C) Cela double automatiquement la bande passante
D) Vagrant fusionne automatiquement les deux sous-réseaux

**Q55.** Le `ping` utilise principalement le protocole :
A) TCP
B) UDP
C) ICMP
D) HTTP

**Q56.** Pourquoi `curl -sk https://IP:6443/healthz` peut renvoyer une erreur 401 Unauthorized tout en prouvant que l'API répond ?
A) Cela prouve que l'API est complètement inaccessible
B) Le serveur a bien reçu et traité la requête HTTP, mais refuse l'accès sans authentification — c'est un signal positif de disponibilité réseau
C) 401 signifie toujours une erreur réseau de bas niveau
D) Cela signifie que le port est fermé

**Q57.** L'adresse `127.0.0.1` désigne :
A) Une adresse publique
B) L'adresse de loopback, qui désigne la machine elle-même
C) L'adresse de broadcast du réseau local
D) Une adresse réservée à Kubernetes uniquement

**Q58.** Dans un environnement Kubernetes, CoreDNS sert à :
A) Allouer des adresses IP aux pods
B) Résoudre les noms des Services Kubernetes en leur ClusterIP interne
C) Chiffrer le trafic entre pods
D) Remplacer kube-proxy

**Q59.** Une route par défaut (`default gateway`) sur une machine signifie :
A) La route à utiliser pour tout le trafic dont la destination ne correspond à aucune route plus spécifique connue
B) La seule route possible vers Internet, toujours statique
C) Une route réservée à DNS uniquement
D) Une route utilisée uniquement par UDP

**Q60.** Pourquoi le port 6443 doit-il être accessible depuis le worker vers le serveur pour qu'un cluster K3s fonctionne ?
A) C'est le port utilisé par SSH
B) C'est le port par défaut de l'API Server Kubernetes/K3s, nécessaire pour la jonction du nœud et la communication continue
C) C'est uniquement pour le DNS
D) C'est uniquement pour le NAT

**Q61.** L'adresse de broadcast d'un réseau `192.168.56.0/24` est :
A) `192.168.56.0`
B) `192.168.56.1`
C) `192.168.56.255`
D) `192.168.255.255`

**Q62.** Combiner `/25` au lieu de `/24` sur le même réseau de base divise le nombre d'adresses disponibles par :
A) 2
B) 4
C) 8
D) Aucun changement

**Q63.** Si un `ping` échoue entre deux VMs mais que `curl` sur un port spécifique fonctionne, l'explication la plus probable est :
A) Le réseau est totalement cassé
B) Le trafic ICMP (ping) est filtré/bloqué alors que le trafic TCP sur ce port précis est autorisé
C) C'est impossible, ping doit toujours réussir si TCP fonctionne
D) Le DNS est mal configuré

**Q64.** Pourquoi est-il déconseillé de donner la même IP statique à deux machines différentes sur le même réseau ?
A) Cela n'a aucune conséquence
B) Cela provoque un conflit d'adresse IP, source de comportements réseau imprévisibles (paquets perdus, accès intermittent)
C) Cela améliore la répartition de charge automatiquement
D) Cela désactive uniquement le DNS

**Q65.** Le terme "routage" désigne :
A) Le chiffrement des paquets réseau
B) Le processus de détermination du chemin que doit suivre un paquet pour atteindre sa destination, à travers un ou plusieurs réseaux
C) L'attribution dynamique d'adresses IP
D) La résolution de noms de domaine

---

## Section 1.D — Linux / Alpine / OpenRC (questions 66-85)

**Q66.** Alpine Linux se distingue notamment par :
A) L'utilisation de la glibc standard et systemd
B) Une taille minimale, l'utilisation de musl libc et BusyBox
C) Le fait d'être basé sur Debian
D) L'absence totale de gestionnaire de paquets

**Q67.** Le gestionnaire de paquets natif d'Alpine Linux est :
A) `apt`
B) `yum`
C) `apk`
D) `pacman`

**Q68.** OpenRC, utilisé par Alpine, est :
A) Un système de fichiers
B) Un système d'init et de gestion de services, alternative à systemd
C) Un langage de script
D) Un protocole réseau

**Q69.** La commande `rc-service k3s status` sert à :
A) Démarrer le service k3s de force
B) Afficher l'état actuel du service k3s géré par OpenRC
C) Supprimer le service k3s
D) Modifier la configuration réseau

**Q70.** BusyBox fournit notamment :
A) Un noyau Linux alternatif
B) Une implémentation compacte de nombreux utilitaires Unix standards (ls, cp, ping...) en un seul binaire
C) Un serveur web complet équivalent à nginx
D) Un système de virtualisation

**Q71.** Sur Alpine avec BusyBox, le shell par défaut `/bin/sh` correspond généralement à :
A) Bash complet
B) `ash` (Almquist shell), une implémentation légère POSIX
C) Zsh
D) PowerShell

**Q72.** La commande `chmod +x script.sh` :
A) Supprime le script
B) Rend le fichier exécutable pour rendre possible son lancement direct
C) Change le propriétaire du fichier
D) Compresse le fichier

**Q73.** `chown user:group fichier` sert à :
A) Changer les permissions de lecture/écriture
B) Changer le propriétaire (utilisateur) et le groupe propriétaire d'un fichier
C) Changer l'extension du fichier
D) Supprimer le fichier

**Q74.** La commande `sudo` permet :
A) De changer définitivement de mot de passe
B) D'exécuter une commande avec les privilèges d'un autre utilisateur, typiquement root
C) De créer un nouvel utilisateur automatiquement
D) De chiffrer un fichier

**Q75.** Un PID (Process ID) est :
A) Un identifiant de paquet réseau
B) Un identifiant unique attribué par le noyau à chaque processus en cours d'exécution
C) Un identifiant de partition disque
D) Un type de permission fichier

**Q76.** La commande `ps aux | grep k3s` permet de :
A) Démarrer le service k3s
B) Filtrer la liste des processus en cours pour ne garder que ceux liés à "k3s"
C) Supprimer tous les processus k3s
D) Vérifier la version de k3s installée

**Q77.** Le système de permissions Unix `rwx` appliqué à un fichier signifie dans l'ordre :
A) Renommer, écrire, exécuter
B) Lecture (read), écriture (write), exécution (execute)
C) Root, write, exit
D) Aucune signification standard

**Q78.** La notation `755` en permissions Unix (chmod 755) correspond à :
A) rwxrwxrwx pour tous
B) rwxr-xr-x (propriétaire: tout, groupe et autres: lecture+exécution)
C) rw-rw-rw-
D) Aucune permission

**Q79.** Un point de montage (mount point) est :
A) Un point d'accès réseau
B) Un répertoire à travers lequel un système de fichiers (disque, partition, dossier partagé...) devient accessible dans l'arborescence
C) Une zone mémoire RAM réservée
D) Un type de processus

**Q80.** La commande `mount | grep vagrant` permet de vérifier :
A) Si le service Vagrant est démarré
B) Si un système de fichiers lié à "vagrant" (comme un dossier partagé vboxsf) est actuellement monté
C) La version de Vagrant installée
D) Les VMs actives sous Vagrant

**Q81.** Sur Alpine, ajouter un dépôt comme `edge/community` dans `/etc/apk/repositories` permet de :
A) Désinstaller Alpine
B) Accéder à des versions plus récentes ou des paquets supplémentaires non présents dans le dépôt stable
C) Activer automatiquement systemd
D) Changer le noyau Linux

**Q82.** Pourquoi `apk add` échoue avec "Permission denied" pour un utilisateur normal ?
A) C'est un bug d'Alpine
B) La modification du système de paquets nécessite des privilèges root, d'où la nécessité de `sudo`
C) Il faut redémarrer la machine
D) `apk` ne fonctionne jamais sans clé SSH

**Q83.** La commande `journalctl -u k3s -n 50` (sur un système systemd) sert à :
A) Démarrer le service k3s
B) Afficher les 50 dernières lignes de logs du service k3s
C) Supprimer les logs de k3s
D) Configurer le démarrage automatique de k3s

**Q84.** Un groupe Unix (comme `docker` ou `vboxusers`) sert à :
A) Remplacer les utilisateurs individuels
B) Donner collectivement certains droits/accès à plusieurs utilisateurs sans dupliquer la configuration
C) Chiffrer les fichiers du groupe
D) Limiter le nombre de processus

**Q85.** La hiérarchie Unix standard place les binaires installés manuellement par l'utilisateur typiquement dans :
A) `/etc/`
B) `/usr/local/bin/`
C) `/var/log/`
D) `/proc/`
...
## Section 1.E — Shell POSIX (questions 86-100)

**Q86.** La différence essentielle entre `/bin/sh` et `/bin/bash` dans un script Alpine est :
A) Aucune différence, ils sont strictement identiques
B) `/bin/sh` sur Alpine est généralement `ash` (POSIX minimal), `bash` propose des extensions non-POSIX absentes par défaut
C) `bash` n'existe pas sous Linux
D) `/bin/sh` est toujours plus rapide qu'une compilation native

**Q87.** Que fait `set -e` au début d'un script shell ?
A) Active le mode debug
B) Arrête immédiatement le script si une commande retourne un code de sortie différent de 0
C) Active l'export automatique de toutes les variables
D) Ignore toutes les erreurs

**Q88.** La syntaxe correcte pour tester si une variable est vide en sh POSIX est :
A) `if [ $VAR = "" ]`
B) `if [ -z "$VAR" ]`
C) `if empty($VAR)`
D) `if VAR == null`

**Q89.** Pourquoi `if [ $? -ne 0]` (sans espace avant `]`) provoque une erreur de syntaxe ?
A) `]` doit toujours être collé au dernier argument en shell
B) En shell, `[` est en réalité une commande, et `]` est son dernier argument ; sans espace, le shell les interprète comme un seul token incorrect
C) `-ne` n'existe pas en POSIX
D) Il manque obligatoirement un point-virgule

**Q90.** Quelle est la sortie de `echo $?` immédiatement après une commande réussie ?
A) Le nom de la dernière commande
B) `0`
C) `1`
D) Le PID du shell

**Q91.** La construction `until condition; do ... done` exécute le bloc :
A) Tant que la condition est vraie
B) Tant que la condition est fausse, et s'arrête dès qu'elle devient vraie
C) Une seule fois, peu importe la condition
D) Jamais, c'est une syntaxe invalide

**Q92.** `cat "$TOKEN"` (avec guillemets et `$`) versus `cat TOKEN` (sans rien) :
A) Sont rigoureusement équivalents
B) Le premier lit le contenu du fichier dont le chemin est stocké dans la variable TOKEN ; le second essaie de lire un fichier littéralement nommé "TOKEN"
C) Le second est la syntaxe recommandée en POSIX
D) Les deux échouent systématiquement

**Q93.** Que fait l'opérateur `|` (pipe) entre deux commandes ?
A) Il exécute les deux commandes en parallèle sans lien
B) Il connecte la sortie standard (stdout) de la première commande à l'entrée standard (stdin) de la seconde
C) Il redirige les erreurs uniquement
D) Il sert uniquement en networking

**Q94.** La redirection `2>&1` signifie :
A) Rediriger stdin vers stdout
B) Rediriger stderr (descripteur 2) vers la même destination que stdout (descripteur 1)
C) Rediriger stdout vers un fichier nommé "1"
D) Fermer le descripteur 2

**Q95.** Un here-document (`<< EOF ... EOF`) sert à :
A) Télécharger un fichier depuis Internet
B) Fournir un bloc de texte multiligne comme entrée standard à une commande, directement dans le script
C) Définir une fonction
D) Chiffrer une variable

**Q96.** Pourquoi écrire `INSTALL_K3S_EXEC="..." sh -` sur une seule ligne logique est important, contrairement à le couper avec un retour à la ligne après les guillemets fermants ?
A) Cela n'a aucune importance, le shell réassemble toujours correctement
B) Si la commande est coupée en deux instructions séparées, le pipe vers `sh -` ne reçoit jamais le flux du `curl`, et `INSTALL_K3S_EXEC` n'est pas transmis à la bonne commande
C) C'est uniquement une question d'esthétique du code
D) Cela change le code de retour mais pas le comportement

**Q97.** La structure `for i in 1 2 3; do ... done` exécute le corps de boucle :
A) Une seule fois
B) Trois fois, avec `i` prenant successivement les valeurs 1, 2 puis 3
C) Une fois par caractère de "123"
D) Indéfiniment

**Q98.** Que retourne `$(commande)` dans un script shell ?
A) Le code de sortie de la commande uniquement
B) La sortie standard (stdout) de la commande, capturée comme une chaîne de caractères utilisable (substitution de commande)
C) Le PID de la commande
D) Rien, cette syntaxe n'existe pas en POSIX

**Q99.** Pourquoi est-il recommandé de toujours mettre les variables entre guillemets doubles (`"$VAR"`) plutôt que nues (`$VAR`) dans les tests `[ ]` ?
A) Cela n'apporte aucune différence pratique
B) Cela évite les erreurs de syntaxe si la variable est vide ou contient des espaces (le test peut sinon recevoir un nombre incorrect d'arguments)
C) Les guillemets sont obligatoires en toutes circonstances en POSIX
D) Cela accélère l'exécution du script

**Q100.** Une fonction shell définie par `nom_fonction() { ... }` :
A) Doit obligatoirement retourner une valeur numérique différente de zéro
B) Regroupe une suite de commandes réutilisables, peut accepter des arguments via `$1`, `$2`... et retourne un code de sortie via `return` ou implicitement
C) Ne peut être appelée qu'une seule fois dans le script
D) Doit toujours être déclarée à la fin du fichier

---

## Section 1.F — Kubernetes / K3s (questions 101-120)

**Q101.** Le rôle principal de l'API Server (`kube-apiserver`) est :
A) Stocker physiquement toutes les données du cluster sur disque
B) Être le point d'entrée unique pour toutes les opérations de lecture/écriture sur l'état du cluster, exposé via une API REST
C) Décider sur quel nœud placer chaque pod
D) Exécuter directement les conteneurs

**Q102.** etcd, dans une installation Kubernetes standard, est :
A) Un cache mémoire temporaire sans persistance
B) Une base de données clé-valeur distribuée qui stocke l'état complet et persistant du cluster
C) Un composant exclusivement réseau
D) Un outil de ligne de commande pour kubectl

**Q103.** Pourquoi K3s utilise SQLite par défaut au lieu d'etcd ?
A) SQLite est obligatoire pour Kubernetes, etcd est une légende
B) SQLite est plus léger en ressources, adapté aux petits clusters/environnements contraints visés par K3s ; etcd reste disponible en option pour la haute disponibilité multi-master
C) etcd ne fonctionne pas sous Linux
D) SQLite est plus rapide qu'etcd dans tous les cas de charge

**Q104.** Le rôle du Scheduler (`kube-scheduler`) est de :
A) Stocker l'état du cluster
B) Décider sur quel nœud worker un nouveau pod doit être placé, en fonction des ressources disponibles et contraintes
C) Exposer l'API REST du cluster
D) Exécuter directement les conteneurs sur les nœuds

**Q105.** Le kubelet :
A) Tourne uniquement sur le control plane
B) Est l'agent qui tourne sur chaque nœud (incluant potentiellement le control plane) et exécute concrètement les ordres reçus de l'API Server pour lancer/surveiller les pods
C) Remplace complètement l'API Server
D) Gère uniquement le stockage réseau

**Q106.** kube-proxy est principalement responsable de :
A) La planification des pods
B) Le maintien des règles réseau permettant aux Services Kubernetes de fonctionner (routage du trafic vers les bons pods)
C) Le stockage de la configuration du cluster
D) La gestion des certificats TLS

**Q107.** Un Pod Kubernetes est :
A) Toujours composé d'un seul conteneur unique sans exception
B) La plus petite unité déployable, pouvant contenir un ou plusieurs conteneurs partageant réseau et stockage
C) Un type de nœud physique
D) Un synonyme strict de "Deployment"

**Q108.** Un Deployment Kubernetes sert à :
A) Définir uniquement le réseau du cluster
B) Décrire l'état désiré d'un ensemble de pods identiques (nombre de réplicas, image, stratégie de mise à jour...) et le maintenir automatiquement
C) Remplacer le besoin de Pods
D) Gérer exclusivement le stockage persistant

**Q109.** Un Service Kubernetes de type `ClusterIP` :
A) Expose le service directement sur Internet
B) Fournit une IP virtuelle stable, accessible uniquement à l'intérieur du cluster, qui redirige vers les pods correspondant à son selector
C) Désigne un type de nœud
D) Ne peut cibler qu'un seul pod à la fois, jamais plusieurs

**Q110.** Un objet Ingress sert à :
A) Remplacer entièrement les Services
B) Définir des règles de routage HTTP/HTTPS (par nom de domaine ou chemin) vers différents Services, à travers un Ingress Controller
C) Stocker les logs du cluster
D) Allouer la RAM aux pods

**Q111.** Sans Ingress Controller (comme Traefik) installé et actif, un objet Ingress créé :
A) Fonctionne immédiatement et route le trafic comme attendu
B) N'a aucun effet concret car rien n'implémente réellement les règles déclarées
C) Bloque tout le trafic du cluster
D) Supprime automatiquement les Services associés

**Q112.** Un Namespace Kubernetes sert à :
A) Améliorer automatiquement les performances réseau
B) Créer une partition logique pour isoler et organiser des ressources au sein d'un même cluster
C) Remplacer le besoin de Deployments
D) Gérer le stockage physique des disques

**Q113.** Un Label Kubernetes (`labels: app: mon-app`) sert à :
A) Définir des permissions de sécurité
B) Attacher une métadonnée clé-valeur identifiante à un objet, utilisable ensuite pour la sélection/filtrage
C) Forcer le placement sur un nœud précis automatiquement
D) Définir le nombre de réplicas

**Q114.** Un `selector.matchLabels` dans un Deployment :
A) Définit où le Deployment est stocké physiquement
B) Indique quels labels les pods gérés par ce Deployment doivent porter, afin que le Deployment puisse les identifier et les administrer
C) Chiffre la communication entre pods
D) Définit le namespace cible automatiquement

**Q115.** Pourquoi `selector.matchLabels` d'un Deployment doit correspondre exactement aux `labels` du `template.metadata` ?
A) Ce n'est pas une obligation réelle
B) Si les labels ne correspondent pas, le Deployment ne peut pas reconnaître/gérer les pods qu'il a lui-même créés, provoquant des comportements incohérents
C) C'est uniquement une convention esthétique sans impact technique
D) Cela ne concerne que les Services, jamais les Deployments

**Q116.** La commande `kubectl get pods -A` :
A) Liste les pods du namespace `default` uniquement
B) Liste les pods de tous les namespaces du cluster
C) Supprime tous les pods
D) Affiche uniquement les pods en erreur

**Q117.** Le `node-token` généré par un serveur K3s sert à :
A) Chiffrer le disque du serveur
B) Authentifier et autoriser un agent (worker) souhaitant rejoindre le cluster contrôlé par ce serveur
C) Identifier uniquement l'utilisateur kubectl
D) Définir l'adresse IP du worker

**Q118.** La variable d'environnement `K3S_URL` utilisée lors de l'installation d'un agent K3s sert à :
A) Définir l'URL du dépôt Git du projet
B) Indiquer au futur agent l'adresse (IP/port 6443) du serveur K3s à rejoindre
C) Configurer le DNS du worker
D) Définir l'image Docker à utiliser

**Q119.** Un pod en état `CrashLoopBackOff` signifie typiquement que :
A) Le pod n'a jamais été planifié sur un nœud
B) Le conteneur démarre puis se termine en erreur de façon répétée, provoquant des tentatives de redémarrage successives par Kubernetes
C) Le pod fonctionne parfaitement mais lentement
D) Il s'agit d'un état uniquement lié au réseau, jamais à l'application elle-même

**Q120.** Une requête `kubectl get endpoints app-one` qui retourne `<none>` indique le plus souvent que :
A) Le Service "app-one" a été supprimé
B) Aucun pod actuellement en état Ready ne correspond au selector du Service "app-one" (problème de labels ou pods non démarrés)
C) Le cluster entier est hors ligne
D) Le namespace n'existe pas

---

*Fin de la Partie 1 — QCM. Passe à la Partie 2 (questions théoriques ouvertes) avant de consulter le corrigé en fin de document.*
# PARTIE 2 — QUESTIONS THÉORIQUES OUVERTES (100 questions)

> Réponds par écrit, de façon complète et précise, comme à l'oral. Évite les réponses approximatives — un évaluateur 42 creuse systématiquement les réponses vagues.

## Section 2.A — Virtualisation et hyperviseurs (Q1-Q12)

**Q1.** Explique la différence entre un hyperviseur de type 1 et de type 2. Donne un exemple de chaque, et situe VirtualBox et KVM dans cette classification.

**Q2.** Pourquoi une machine virtuelle a-t-elle besoin de la virtualisation matérielle (VT-x/AMD-V) pour fonctionner de façon performante ? Que se passe-t-il en son absence ?

**Q3.** Explique la relation entre KVM, QEMU et libvirt. Pourquoi ces trois composants existent-ils séparément plutôt que d'être fusionnés en un seul logiciel ?

**Q4.** Quelle est la différence fondamentale entre la virtualisation (VM) et la conteneurisation (Docker) au niveau du noyau du système d'exploitation ?

**Q5.** Pourquoi peut-on dire que les VMs offrent une isolation plus forte que les conteneurs, mais avec un coût en ressources plus élevé ?

**Q6.** Explique ce qu'est un "Storage pool" dans libvirt, et où sont stockées par défaut les images disque des VMs gérées par libvirt.

**Q7.** Pourquoi les VirtualBox Guest Additions doivent-elles, dans l'idéal, correspondre à la version de VirtualBox installée sur l'hôte ? Donne un exemple concret de dysfonctionnement observé dans le projet quand ce n'est pas le cas.

**Q8.** Explique ce qu'est `vboxsf` et son rôle exact dans le mécanisme des dossiers partagés Vagrant/VirtualBox.

**Q9.** Si tu observes qu'une même machine a à la fois VirtualBox et libvirt installés, comment expliques-tu à un évaluateur lequel des deux Vagrant a utilisé pour une VM donnée, et comment le vérifier concrètement ?

**Q10.** Pourquoi l'allocation de RAM et de CPU à une VM dans un Vagrantfile n'est-elle pas une réservation strictement exclusive de ressources physiques (sauf configuration spécifique) ?

**Q11.** Qu'est-ce qu'un disque virtuel (par exemple un fichier `.vdi`) et comment se comporte-t-il par rapport à un vrai disque dur en termes d'allocation d'espace (différence entre allocation dynamique et fixe) ?

**Q12.** Explique pourquoi exécuter plusieurs VMs avec K3s peut consommer significativement plus de ressources que le même nombre de conteneurs K3d sur la même machine.

---

## Section 2.B — Vagrant (Q13-Q26)

**Q13.** Explique précisément le rôle du fichier `Vagrantfile` et pourquoi on dit que Vagrant fonctionne sur un principe d'"infrastructure as code".

**Q14.** Que se passe-t-il concrètement en arrière-plan quand tu exécutes `vagrant up` pour la toute première fois sur un Vagrantfile donné, étape par étape ?

**Q15.** Quelle est la différence entre `vagrant halt`, `vagrant suspend` et `vagrant destroy` en termes d'état conservé et de ressources libérées ?

**Q16.** Explique le rôle exact du dossier `.vagrant/` créé localement, et ce qui se passerait si tu le supprimais manuellement alors que la VM existe encore dans VirtualBox.

**Q17.** Pourquoi Vagrant remplace-t-il automatiquement la clé SSH "insecure" par défaut par une nouvelle clé générée à chaque création de VM ?

**Q18.** Qu'est-ce qu'un "synced folder" (dossier partagé) Vagrant, et quels sont les différents types possibles (`virtualbox`, `rsync`, `nfs`...) ? Donne un avantage et un inconvénient de chacun.

**Q19.** Dans le contexte du projet, explique pourquoi `/vagrant` peut ne pas être monté correctement malgré une configuration apparemment correcte, et liste au moins deux causes racines distinctes possibles.

**Q20.** Quelle est la différence entre `vagrant provision` et `vagrant reload --provision` ? Dans quel cas utiliserais-tu l'un plutôt que l'autre lors du débogage ?

**Q21.** Explique pourquoi définir `config.vm.box_check_update = false` peut être pertinent dans un contexte d'évaluation/soutenance où une connexion réseau fiable n'est pas garantie.

**Q22.** Comment Vagrant détermine-t-il quel provider (VirtualBox, libvirt...) utiliser pour une VM si plusieurs sont installés sur la machine hôte, en l'absence de spécification explicite ?

**Q23.** Pourquoi est-il recommandé d'allouer le minimum de ressources nécessaire (RAM/CPU) à chaque VM Vagrant dans ce projet, plutôt que le maximum disponible ?

**Q24.** Explique le rôle des scripts de provisioning de type `shell` dans un Vagrantfile, et pourquoi ils sont exécutés en tant que root par défaut.

**Q25.** Que se passe-t-il si un script de provisioning échoue (code de sortie différent de 0) au milieu de l'exécution de `vagrant up` portant sur plusieurs VMs définies dans le même Vagrantfile ?

**Q26.** Pourquoi peut-il être pertinent, dans un environnement de test sans accès root sur la machine hôte (comme certaines machines 42), d'utiliser un SSD externe avec son propre OS plutôt que d'installer Vagrant directement sur le système de l'école ?

---

## Section 2.C — Réseau (Q27-Q42)

**Q27.** Explique la différence entre un réseau NAT et un réseau Host-only dans le contexte de Vagrant/VirtualBox, en précisant qui peut communiquer avec qui dans chaque cas.

**Q28.** Pourquoi le projet IoT impose des adresses IP fixes (192.168.56.110 et .111) plutôt que de laisser DHCP les attribuer dynamiquement ?

**Q29.** Explique ce qu'est un masque de sous-réseau et pourquoi il est indissociable d'une adresse IP pour en déduire la portée du réseau local.

**Q30.** Que signifie exactement la notation CIDR `/24`, et comment calcule-t-on le nombre d'adresses hôtes disponibles à partir de ce préfixe ?

**Q31.** Explique pourquoi deux machines avec des adresses IP appartenant à des sous-réseaux différents (ex: 192.168.56.10/24 et 192.168.57.10/24) ne peuvent pas communiquer directement sans passer par un routeur/gateway.

**Q32.** Qu'est-ce qu'une "gateway" (passerelle) et quel est son rôle quand une machine veut atteindre une adresse IP hors de son réseau local ?

**Q33.** Explique la différence fondamentale entre TCP et UDP, et pourquoi l'API Kubernetes utilise TCP plutôt qu'UDP.

**Q34.** Qu'est-ce qu'un port en réseau, et pourquoi deux services différents peuvent-ils tourner sur la même adresse IP sans conflit ?

**Q35.** Explique ce qu'est une socket réseau et donne sa définition formelle (les éléments qui la composent).

**Q36.** Dans le projet IoT Partie 2, explique précisément comment l'en-tête HTTP `Host` permet à un seul Ingress, sur une seule adresse IP et un seul port, de distinguer trois applications différentes.

**Q37.** Pourquoi `ping` peut échouer entre deux machines alors que `curl` sur un port TCP spécifique fonctionne parfaitement ? Donne une explication technique précise.

**Q38.** Qu'est-ce que DHCP, et pourquoi ce mécanisme n'est volontairement pas utilisé pour les IPs du cluster K3s dans la Partie 1 du projet ?

**Q39.** Qu'est-ce que DNS, et quel composant Kubernetes en assure le rôle à l'intérieur d'un cluster ?

**Q40.** Explique pourquoi une erreur HTTP 401 (Unauthorized) renvoyée par un serveur, bien que ce soit techniquement une "erreur", constitue en réalité une preuve que la connectivité réseau de bas niveau fonctionne.

**Q41.** Dans le contexte de la jonction worker-serveur K3s, explique pourquoi il est nécessaire que le port 6443 soit accessible depuis le worker vers le serveur, et ce qui se produirait concrètement si un pare-feu bloquait ce port.

**Q42.** Explique la notion de routage et pourquoi, sur un simple réseau privé Vagrant à deux machines, aucun routeur dédié n'est nécessaire pour qu'elles communiquent.

---

## Section 2.D — Linux / Alpine / Système (Q43-Q54)

**Q43.** Pourquoi Alpine Linux est-elle un choix pertinent comme système d'exploitation invité pour des VMs destinées à faire tourner K3s, en comparaison avec Ubuntu par exemple ?

**Q44.** Qu'est-ce que musl libc, et en quoi diffère-t-elle de la glibc utilisée par la majorité des distributions Linux traditionnelles ?

**Q45.** Explique le rôle d'OpenRC sur Alpine Linux, et en quoi son fonctionnement diffère de celui de systemd qu'on trouve sur Ubuntu/Debian.

**Q46.** Pourquoi la commande `systemctl status k3s` échoue-t-elle sur Alpine Linux, et quelle commande faut-il utiliser à la place ?

**Q47.** Explique la différence entre les permissions `rwx` appliquées à un fichier et celles appliquées à un répertoire (le sens de "exécution" change-t-il ?).

**Q48.** Pourquoi `sudo` est-il nécessaire pour exécuter certaines commandes comme `apk add` ou `mount`, et que représente concrètement ce changement de privilèges au niveau du système ?

**Q49.** Explique ce qu'est un point de montage, et donne un exemple concret tiré du projet où un montage incorrect ou absent (`/vagrant`) a provoqué une panne en cascade.

**Q50.** Qu'est-ce qu'un PID, et pourquoi deux exécutions successives du même programme reçoivent-elles des PID différents ?

**Q51.** Explique la différence entre un processus et un service (au sens OpenRC/systemd).

**Q52.** Pourquoi consulter les logs systèmes (`/var/log/messages`, `journalctl`, ou les logs spécifiques comme `/var/log/k3s.log`) est-il une étape indispensable du débogage plutôt que de se fier uniquement aux messages affichés par Vagrant ?

**Q53.** Explique ce que signifie techniquement le fait qu'un module noyau comme `vboxguest` soit "chargé" (visible via `lsmod`), et pourquoi cela ne garantit pas pour autant que le dossier partagé fonctionne.

**Q54.** Pourquoi est-il pertinent, lors du débogage d'un problème de VM, de tester une commande manuellement en SSH avant de modifier un script de provisioning automatisé ?

---

## Section 2.E — Shell et scripting (Q55-Q68)

**Q55.** Explique la différence entre `sh` (POSIX, type ash sur Alpine) et `bash`, et pourquoi cette distinction est cruciale pour écrire des scripts de provisioning portables.

**Q56.** Que fait exactement `set -e` au début d'un script, et donne un exemple où son absence pourrait masquer une erreur critique dans le contexte du projet.

**Q57.** Explique la mécanique exacte de la substitution de commande `$(commande)` et donne un exemple tiré des scripts du projet.

**Q58.** Pourquoi faut-il systématiquement mettre les variables entre guillemets doubles dans les tests (`[ "$VAR" = "valeur" ]`) plutôt que nues, et quel bug concret cela évite-t-il ?

**Q59.** Explique la différence entre les boucles `while` et `until` en shell, avec un exemple où l'une serait plus naturelle que l'autre.

**Q60.** Qu'est-ce qu'un "exit code" (code de sortie) d'une commande, et comment l'accède-t-on immédiatement après l'exécution d'une commande ?

**Q61.** Explique la différence entre stdout et stderr, et pourquoi il peut être utile de les rediriger séparément lors du débogage d'un script.

**Q62.** Que signifie la redirection `2>/dev/null`, et dans quel contexte du projet cette pratique a-t-elle été utilisée (et est-ce toujours une bonne idée) ?

**Q63.** Explique le fonctionnement d'un here-document (`<<EOF ... EOF`), notamment la différence de comportement entre `<<EOF` et `<<'EOF'` concernant l'interpolation des variables.

**Q64.** Pourquoi écrire une commande en pipe (`commande1 | commande2`) sur deux lignes séparées par un retour à la ligne mal placé (sans `\` de continuation) peut casser totalement la logique voulue ? Illustre avec l'exemple du bug `INSTALL_K3S_EXEC` rencontré dans le projet.

**Q65.** Explique ce qu'est une fonction shell, comment on lui passe des arguments, et comment elle communique un résultat (au-delà du simple affichage).

**Q66.** Pourquoi la boucle `until kubectl get nodes > /dev/null 2>&1; do ... done` est-elle une stratégie courante pour attendre la disponibilité d'un service avant de continuer un script ?

**Q67.** Explique l'erreur classique `[ $? -ne 0]` (sans espace) et pourquoi le shell ne peut pas la "deviner" même si l'intention est claire pour un humain.

**Q68.** Quelle différence fais-tu entre `[ ]` (test POSIX) et `[[ ]]` (extension bash) ? Pourquoi cette dernière ne doit-elle pas être utilisée dans un script `#!/bin/sh` destiné à Alpine ?

---

## Section 2.F — Ruby et Vagrantfile (Q69-Q76)

**Q69.** Explique la syntaxe `Vagrant.configure("2") do |config| ... end` : que représente le bloc `do...end`, et que représente la variable `config` ?

**Q70.** Qu'est-ce que l'interpolation de chaîne `#{}` en Ruby, et donne un exemple tiré d'un Vagrantfile du projet où elle est utilisée pour construire dynamiquement un nom de machine.

**Q71.** Explique pourquoi `config.vm.define SERVER_NAME do |server| ... end` crée un nouveau contexte de configuration limité à cette VM spécifique, distinct du `config` global.

**Q72.** Quelle est la différence entre une variable Ruby classique (`LOGIN = "ankammer"`) et l'usage de cette variable à l'intérieur d'un bloc imbriqué dans le même Vagrantfile ?

**Q73.** Explique la syntaxe `server.vm.provider "virtualbox" do |vb| ... end` et pourquoi elle est nécessaire pour personnaliser la RAM/CPU plutôt que de le faire au niveau global.

**Q74.** Pourquoi la position de `config.vm.synced_folder` dans le Vagrantfile (au niveau global vs. à l'intérieur d'un bloc `define`) a-t-elle un impact direct sur le fonctionnement du dossier partagé sur chaque VM ?

**Q75.** Que se passe-t-il si tu oublies un `end` dans un Vagrantfile comportant plusieurs blocs imbriqués (`Vagrant.configure`, `config.vm.define`, `provider`) ? Quel type d'erreur cela génère-t-il ?

**Q76.** Explique la différence entre `server.vm.provision "shell", path: "scripts/server.sh"` et `server.vm.provision "shell", inline: "..."`.

---

## Section 2.G — Kubernetes et K3s (Q77-Q92)

**Q77.** Explique la différence fondamentale entre Kubernetes (K8s) "standard" et K3s, en citant au moins trois différences concrètes.

**Q78.** Pourquoi dit-on que Kubernetes fonctionne selon un modèle "déclaratif" plutôt qu'"impératif" ? Illustre avec un exemple concret de Deployment.

**Q79.** Explique le rôle exact de chacun des quatre composants principaux du Control Plane : API Server, etcd (ou SQLite pour K3s), Scheduler, Controller Manager.

**Q80.** Pourquoi un Pod est-il considéré comme "éphémère", et quelles conséquences cela a-t-il sur la façon dont on doit concevoir l'accès réseau à une application (pourquoi ne jamais cibler directement l'IP d'un pod) ?

**Q81.** Explique pas à pas comment un Deployment garantit qu'il y a toujours exactement N réplicas d'un pod en fonctionnement, même en cas de panne.

**Q82.** Qu'est-ce qu'un Rolling Update, et comment les paramètres `maxSurge` et `maxUnavailable` permettent-ils de garantir une mise à jour sans interruption de service ?

**Q83.** Explique pourquoi un Service Kubernetes est nécessaire même si les pods qu'il cible ont déjà chacun leur propre adresse IP.

**Q84.** Détaille la chaîne complète de résolution d'une requête HTTP entrante dans la Partie 2 du projet, depuis `curl -H "Host:app1.com" http://192.168.56.110` jusqu'à la réponse du pod final.

**Q85.** Pourquoi le sujet exige spécifiquement 3 réplicas pour l'application 2 et pas pour les deux autres ? Qu'est-ce que cela permet de démontrer techniquement ?

**Q86.** Explique le rôle de l'annotation `kubernetes.io/ingress.class: "traefik"` dans un manifeste Ingress, et ce qui se passerait en son absence si plusieurs Ingress Controllers étaient installés.

**Q87.** Qu'est-ce qu'un `node-token` dans K3s, où est-il stocké sur le serveur, et pourquoi sa confidentialité est-elle importante en production (même si elle compte moins dans un contexte pédagogique local) ?

**Q88.** Explique précisément la séquence d'événements depuis l'exécution du script worker.sh jusqu'à l'apparition du worker dans `kubectl get nodes` sur le serveur.

**Q89.** Pourquoi est-il essentiel que le worker et le serveur K3s utilisent exactement la même version de K3s (`INSTALL_K3S_VERSION`) ?

**Q90.** Qu'est-ce que la commande `kubectl get nodes -o wide` apporte de plus par rapport à `kubectl get nodes` simple ?

**Q91.** Explique pourquoi un cluster K3s avec une seule VM (1 CPU, 1 Go de RAM) peut connaître des ralentissements importants (requêtes SQL lentes, timeouts) avec les versions très récentes de K3s, et quelles solutions concrètes permettent d'y remédier.

**Q92.** Dans la Partie 3 du projet (Argo CD), explique la différence entre `prune: true` et `selfHeal: true` dans la `syncPolicy` d'une Application Argo CD.

---

## Section 2.H — YAML et manifestes (Q93-Q100)

**Q93.** Pourquoi l'indentation en YAML doit-elle impérativement être réalisée avec des espaces et jamais des tabulations ?

**Q94.** Explique la différence de syntaxe entre une liste YAML et un dictionnaire (map) YAML, avec un exemple de chacun tiré d'un manifeste Kubernetes.

**Q95.** Que représente le séparateur `---` dans un fichier YAML contenant plusieurs manifestes Kubernetes (comme un Deployment suivi d'un Service dans le même fichier) ?

**Q96.** Pourquoi écrire `replicas: "3"` (avec guillemets) au lieu de `replicas: 3` peut-il provoquer une erreur de validation dans certains champs Kubernetes qui attendent strictement un entier ?

**Q97.** Explique pourquoi les champs `selector.matchLabels` (dans un Deployment) et `labels` (dans le `template.metadata`) doivent être identiques, en termes de conséquence si ce n'est pas le cas.

**Q98.** Qu'est-ce qu'un bloc littéral multi-lignes en YAML introduit par le caractère `|`, et donne un exemple où cela est utile dans un manifeste Kubernetes (par exemple un script inline dans un conteneur).

**Q99.** Pourquoi une simple erreur d'indentation d'un seul caractère dans un fichier YAML Kubernetes peut-elle complètement changer la structure interprétée (faire passer un champ d'enfant à frère, par exemple) ?

**Q100.** Explique pourquoi il est recommandé de toujours valider un manifeste YAML (par exemple avec `kubectl apply --dry-run=client -f fichier.yaml`) avant de l'appliquer réellement sur un cluster en production.

---

*Fin de la Partie 2. Passe à la Partie 3 (analyse de code shell) avant de consulter le corrigé.*
# PARTIE 3 — ANALYSE DE CODE SHELL (30 exercices)

> Pour chaque script : (1) explique ce qu'il est censé faire, (2) identifie toutes les erreurs présentes, (3) propose la version corrigée. Les scripts sont volontairement bugués, à la manière de scripts réels produits pendant le développement du projet.
...
echo "K3s installed"
```
Que fait réellement ce script (pas ce qu'il est censé faire) ? Identifie l'erreur structurelle critique.
...
```
Quel est le problème de logique dans la gestion d'erreur ici ?
...
```
Deux bugs distincts à identifier dans cet extrait.
...
```
Quelle redirection est mal écrite ? Quel est le comportement réel observé (pas celui voulu) ?
...
until curl -sk https://${SERVER_IP}/:6443/healthz | grep -q "ok"; do
    TRIES=$((TRIES + 1))
    if [ TRIES -ge 12 ]; then
        echo "Error: K3s API unavailable!"
        exit 1
    fi
    sleep 5
done
```
Deux bugs : un dans l'URL, un dans la condition. Identifie-les précisément.
...
```
Identifie l'erreur de syntaxe précise et explique pourquoi le shell ne peut pas l'interpréter correctement.
...
echo "Token copied"
```
Dans le contexte d'un script exécuté sur une VM avec un Vagrantfile ne déclarant pas `synced_folder` explicitement, ce script présente un problème conceptuel grave bien qu'il ne contienne aucune erreur de syntaxe. Lequel ?
...
```
Ce script mélange plusieurs commandes système incompatibles. Identifie-les et explique pourquoi cela ne peut pas fonctionner sur Alpine Linux.
...
if [ ! -d "/vagrant" ] || [ -z "$(ls -A /vagrant 2>/dev/null)" ]; then
    mkdir -p /vagrant
    mount -t virtiofs vagrant /vagrant 2>/dev/null ||
    mount -t 9p -o trans=virtio,version=9p2000.L vagrant /vagrant 2>/dev/null ||
    echo ">>> Mount failed, continuing anyway..."
fi
```
Avec `set -e` activé en haut du script, ce bloc particulier pose-t-il un risque ? Justifie ta réponse en expliquant le comportement des `||` chaînés sous `set -e`.
...
```
Ce script fonctionne globalement, mais présente un défaut de précision/affichage. Lequel, et pourquoi peut-il induire en erreur quelqu'un qui débogue ensuite ?
...
```
Exécuté directement par un utilisateur non-root via le provisioning Vagrant standard, ce script peut tout de même échouer dans certains contextes. Lequel, et que faudrait-il vérifier ?
...
```
Ce script est globalement correct. Quelle amélioration de robustesse manque-t-il pour diagnostiquer la cause exacte d'un échec après les 20 tentatives ?
...
docker run -d -p 8888:8888 myapp:$VERSION
curl http://localhost:8888/
```
Ce script ne contient pas de bug shell à proprement parler, mais une faille logique de séquencement temporel qui causera un échec intermittent. Identifie-la.
...
```
Que va réellement afficher la commande `echo "vagrant ssh "${NAME}"S"` ? Le résultat est-il celui attendu ?
...
echo "${SERVER_IP%.*}S"
```
Que produit cette expression exactement ? Explique le mécanisme du paramètre `%.*` et pourquoi le résultat n'a pas de sens dans ce contexte.
...
```sh
#!/bin/sh
echo "Token partial preview"
head -c 50 /vagrant/node-token
echo "..."
TOKEN_LENGTH=$(echo -n "$NODE_TOKEN" | wc -c)
echo "Length: $TOKEN_LENGTH chars"
```
Ce script fonctionne, mais une variable utilisée ici n'a peut-être pas été définie plus haut dans le contexte d'un script réel. Quel risque cela représente-t-il avec et sans `set -u` ?
...
```
Ce script est syntaxiquement correct. Quelle hypothèse implicite sur l'état du cluster pourrait le faire échouer silencieusement (sans `set -e`) ?
...
```sh
#!/bin/sh
echo ">>> Waiting for token at /vagrant/node-token..."
TOKEN_URL="http://${SERVER_IP}:9999/node-token"
TOKEN_FILE="/tmp/node-token"
while true; do
    if curl -sf --connect-timeout 3 "${TOKEN_URL}" -o "${TOKEN_FILE}" 2>/dev/null; then
...
```
Ce script contient une boucle infinie volontaire (`while true`). Quel risque cela présente-t-il concrètement en environnement de provisioning automatisé (Vagrant) si le serveur ne démarre jamais correctement ?
...
#!/bin/sh
cd /tmp/token-share
nohup python3 -m http.server 9999 > /tmp/http-token.log 2>&1 &
echo "Token server started, PID: $!"
```
Explique précisément ce que fait `nohup` ici, ce que représente `$!`, et pourquoi le `&` est indispensable dans ce contexte de script de provisioning.
...
```sh
#!/bin/sh
echo "Removing old K3s installation..."
/usr/local/bin/k3s-killall.sh
rm -rf /var/lib/rancher/k3s/
echo "Reinstalling K3s..."
curl -sfL https://get.k3s.io | sh -
```
Ce script semble correct pour un reset complet, mais une commande capitale manque en fin de script pour qu'un nouveau cluster propre soit réellement opérationnel après. Laquelle, et pourquoi ?

---

### Exercice 3.21 — Complétion de script

Complète les parties manquantes (marquées `# TODO`) de ce script pour qu'il attende correctement le token avant de continuer, avec un timeout de 5 minutes :
...
# TODO : écrire la boucle d'attente avec timeout

echo "Token trouvé"
NODE_TOKEN=$(# TODO : lire le contenu du fichier sans le retour à la ligne final
)
```

---

### Exercice 3.22 — Complétion de script

Complète ce script pour qu'il teste la connectivité réseau avec 3 tentatives avant d'abandonner :
...
# TODO : boucle de 3 tentatives de ping avec 2 secondes d'attente entre chaque
...
### Exercice 3.23 — Détection d'erreur dans un contexte plus large
...
```
Au-delà des bugs de syntaxe (il n'y en a pas ici), quelle hypothèse d'environnement ce script fait-il qui pourrait le faire échouer entièrement sur Alpine ?
...
```
Ce script a été écrit pour une version de K3s antérieure à la v1.26. Pourquoi peut-il bloquer indéfiniment (jusqu'au timeout) avec K3s v1.35, même si le serveur fonctionne parfaitement ?

---

### Exercice 3.25 — Script avec heredoc
...
HTMLEOF
```
Le développeur s'attendait à voir le vrai nom d'hôte affiché dans la page HTML générée. Ce ne sera pas le cas. Explique pourquoi, en lien avec le type de guillemets utilisé autour du délimiteur `HTMLEOF`.
...
cp "$TOKEN_FILE" /vagrant/node-token
```
Ce script omet une vérification essentielle avant la dernière ligne, qui causera une erreur cryptique ("No such file or directory") si exécuté trop tôt après le démarrage de K3s. Laquelle ?

---

### Exercice 3.27 — Analyse comparative

Compare ces deux façons d'attendre l'API K3s et explique laquelle est la plus robuste face à un changement de version de K3s, en justifiant techniquement :
...
**Version B :**
```sh
until curl -sk --max-time 3 "https://${SERVER_IP}:6443" 2>/dev/null | grep -qE "Unauthorized|ok|Bad Request"; do
    sleep 5
done
```
...
```sh
#!/bin/sh
echo "Démarrage du serveur HTTP pour le token..."
apk add --no-cache python3
mkdir -p /tmp/token-share
cp /var/lib/rancher/k3s/server/node-token /tmp/token-share/node-token
cd /tmp/token-share
python3 -m http.server 9999 > /tmp/http-token.log 2>&1
echo "Serveur démarré"
```
Pourquoi le message "Serveur démarré" ne s'affichera-t-il jamais dans ce script, contrairement à l'intention évidente du développeur ?

---

### Exercice 3.29 — Script complet à corriger entièrement

Identifie TOUTES les erreurs (il y en a au moins 6) dans ce script worker complet :
...
### Exercice 3.30 — Script Argo CD / mise à jour de version

```sh
#!/bin/sh
echo "Mise à jour de la version applicative..."
...
echo "Mise à jour poussée"
```
Identifie deux erreurs de syntaxe distinctes (une dans la commande `sed`, une dans la commande `git commit`) qui empêcheront ce script de fonctionner tel qu'écrit.

---

*Fin de la Partie 3. Passe à la Partie 4 (Vagrantfile / Ruby) avant de consulter le corrigé.*
# PARTIE 4 — ANALYSE DE VAGRANTFILE / RUBY (30 exercices)

> Pour chaque extrait : explique ligne par ligne ce qu'il fait, identifie les erreurs s'il y en a, et prédis le comportement réel observé lors d'un `vagrant up`.
...
```
Explique chaque token de la première ligne : `Vagrant`, `configure`, `"2"`, `do |config|`. Que se passe-t-il si on oublie le `end` final ?
...
puts SERVER_NAME
```
Que va afficher `puts SERVER_NAME` ? Explique le mécanisme exact de `#{LOGIN}`.
...
```
Pourquoi utilise-t-on `server.vm.hostname` ici et non `config.vm.hostname`, alors que `config` est accessible dans tout le fichier ?
...
```
Identifie les deux arguments passés à la méthode `network`. Le second argument utilise une syntaxe particulière de Ruby : laquelle, et que signifie le symbole `ip:` ?
...
```
Si ce bloc est placé À L'EXTÉRIEUR de tout `config.vm.define`, quelle conséquence cela a-t-il sur le nombre de VMs concernées par ces réglages ?
...
```
Ce Vagrantfile contient une erreur de syntaxe Ruby évidente. Identifie-la précisément (compte les `do` et les `end`).
...
```
Place ce code AVANT les blocs `config.vm.define`. Explique pourquoi son emplacement (avant, et non à l'intérieur d'un bloc `define` spécifique) lui donne une portée différente.
...
```
Ce bloc ne déclare PAS `synced_folder` explicitement contrairement au serveur. Si le serveur a `config.vm.synced_folder` déclaré globalement (avant les define), ce worker en hérite-t-il automatiquement ? Justifie.
...
```
Le chemin `"scripts/server.sh"` est-il relatif au dossier où se trouve le Vagrantfile, ou relatif au dossier `/vagrant` dans la VM ? Ces deux interprétations sont-elles équivalentes en pratique ?
...
config.vm.define WORKER_NAME do |worker|
    worker.vm.network "private_network", ip: SERVER_IP
end
```
Bug volontaire ici (recopie). Quelle est la conséquence concrète de ce bug lors du `vagrant up` ?
...
```
Si `box_check_update` était laissé à sa valeur par défaut (`true`), quel impact concret cela aurait-il sur le temps d'exécution d'un `vagrant up`, particulièrement en environnement de soutenance où la connexion réseau peut être instable ?
...
```
Explique la structure conditionnelle Ruby ici (`if ... end`). Que se passe-t-il concrètement si le plugin `vagrant-vbguest` n'est pas installé sur la machine qui exécute ce Vagrantfile ?
...
config.vm.define WORKER_NAME do |worker|
```
Repère une erreur de syntaxe Ruby dans cet extrait (incomplet volontairement à la fin). Indice : compte les `end`.
...
vb.memory = "1024"
```
Cette ligne, comparée à `vb.memory = 1024` (sans guillemets), pose-t-elle un problème ? Quelle est la différence de type Ruby entre les deux écritures ?
...
```
Ces deux lignes sont-elles exécutées dans un ordre garanti ? Si oui, lequel, et pourquoi cet ordre est-il important dans le contexte du projet (Partie 2 notamment) ?
...
```
Le sujet du projet impose que le nom de la première machine soit suivi de la lettre S, et la seconde de SW. Si `LOGIN` contenait par erreur un espace ("ankammer 42"), quel problème concret cela engendrerait-il pour `config.vm.hostname` ?
...
```
Deux affectations successives à `config.vm.box`. Que se passe-t-il réellement (quelle est la valeur finale utilisée) ?
...
```
Que fait précisément `vb.gui = true` ? Dans quel contexte de débogage cette option serait-elle utile, malgré le fait qu'elle ne soit jamais utilisée dans le projet final ?
...
```
Cette ligne n'apparaît dans aucun des Vagrantfiles du projet IoT. Explique ce qu'elle ferait si elle était ajoutée, et pourquoi ce mécanisme (port forwarding) est différent de `private_network`.
...
```
Cette ligne est placée tout en haut, avant `Vagrant.configure`. Explique son rôle dans un contexte où libvirt et VirtualBox sont tous les deux installés sur la machine hôte.

---

### Exercice 4.21 — Complétion

Complète ce Vagrantfile pour qu'il définisse une seconde machine nommée "WORKER_NAME" avec l'IP `WORKER_IP`, 512 Mo de RAM, et exécutant le script `scripts/worker.sh` :
...
Vagrant.configure("2") do |config|
  ...
  # TODO : complète ici la définition du WORKER
...
config.vm.define SERVER_NAME do |server2|
  server2.vm.network "private_network", ip: SERVER_IP
end
```
Deux blocs `config.vm.define` utilisent le même nom `SERVER_NAME`. Quel est le comportement de Vagrant face à cette duplication ?
...
```ruby
server.vm.provision "shell", path: "scripts/server.sh", run: "always"
```
Que change l'option `run: "always"` par rapport à l'absence de cette option (comportement par défaut) ? Dans quel cas ce paramètre serait-il dangereux pour le script `server.sh` du projet (qui réinstalle K3s à chaque fois) ?
...
```ruby
worker.vm.provision "shell", path: "scripts/worker.sh"
worker.vm.provision "shell", inline: "echo Done"
```
Combien de scripts de provisioning distincts sont exécutés ici pour cette VM ? Sont-ils exécutés dans l'ordre d'écriture ?
...
vb.memory = 1024
```
Trois lignes dans le même bloc provider. Quelle sera la RAM finale réellement allouée à la VM ?

---

### Exercice 4.26 — Prédiction de comportement

Voici un Vagrantfile complet. Prédis précisément ce qui se passera à l'exécution de `vagrant up`, ligne par ligne si nécessaire :
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "machineB" do |b|
    ...
    b.vm.network "private_network", ip: "192.168.56.100"
  end
end
```
...
```
Explique la méthode `vb.customize` et la signification du symbole `:id` dans ce contexte (que représente-t-il concrètement à l'exécution) ?
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "ankammerS" do |server|
    server.vm.box = "generic/alpine317"
    server.vm.hostname = "ankammerS"
  end

  config.vm.define "ankammerSW" do |worker|
    worker.vm.box = "generic/alpine319"
    worker.vm.hostname = "ankammerSW"
  end

end
```
Ici, `config.vm.box` n'est jamais défini globalement, mais chaque VM définit sa propre box (différente l'une de l'autre !). Est-ce une syntaxe valide en Ruby/Vagrant ? Quelle conséquence cela a-t-il sur la cohérence du cluster K3s prévu ensuite ?

---

### Exercice 4.29 — Détection d'erreur complète

Identifie TOUTES les erreurs dans ce Vagrantfile (au moins 3) :
...
### Exercice 4.30 — Synthèse : explication ligne par ligne complète

Explique intégralement, ligne par ligne, ce Vagrantfile final tel qu'il pourrait être présenté en soutenance, en justifiant chaque choix technique (pourquoi cette valeur de RAM, pourquoi ce type de réseau, pourquoi ce provisioning) :
...
*Fin de la Partie 4. Passe à la Partie 5 (Réseau) avant de consulter le corrigé.*
# PARTIE 5 — RÉSEAU (50 exercices)

> Calculs, schémas et analyses. Pour les calculs CIDR, montre ton raisonnement complet, pas seulement le résultat final.

## Section 5.A — Calculs CIDR et sous-réseaux (Q1-Q15)

**Q1.** Donne le masque décimal pointé correspondant à chacun des préfixes CIDR suivants : `/8`, `/16`, `/24`, `/25`, `/30`.

**Q2.** Pour le réseau `192.168.56.0/24` : donne l'adresse réseau, l'adresse de broadcast, et la plage d'adresses utilisables pour des hôtes.

**Q3.** Pour le réseau `192.168.1.0/26` : combien d'adresses hôtes utilisables ce sous-réseau offre-t-il ? Détaille le calcul.

**Q4.** Les adresses `192.168.56.110` et `192.168.56.111` (utilisées dans le projet IoT) appartiennent-elles au même sous-réseau si le masque est `/24` ? Justifie avec un calcul.

**Q5.** Si on changeait le masque du réseau du projet de `/24` à `/29`, ces deux mêmes adresses (.110 et .111) seraient-elles toujours dans le même sous-réseau ? Démontre-le par le calcul.

**Q6.** Découpe le réseau `10.0.0.0/24` en 4 sous-réseaux de taille égale. Donne le préfixe CIDR résultant et la plage de chaque sous-réseau.

**Q7.** Combien d'adresses hôtes utilisables fournit un réseau en `/30` ? Pourquoi ce préfixe est-il classiquement utilisé pour des liaisons point-à-point ?

**Q8.** L'adresse `192.168.56.256` est-elle une adresse IPv4 valide ? Justifie.

**Q9.** Convertis l'adresse IP `192.168.56.110` en notation binaire (les 4 octets).

**Q10.** Un évaluateur te donne l'adresse `172.16.5.200/22`. Calcule l'adresse réseau de ce sous-réseau.

**Q11.** Pour le réseau `192.168.56.0/24`, indique précisément si l'adresse `192.168.56.0` elle-même peut être attribuée à une machine hôte. Justifie.

**Q12.** Quelle est la plage d'adresses IP privées de classe C selon la RFC 1918, et le préfixe CIDR habituellement associé pour la couvrir entièrement ?

**Q13.** Pourquoi un préfixe `/32` désigne-t-il une seule et unique adresse IP, sans aucun hôte "utilisable" au sens réseau classique ?

**Q14.** Si tu observes l'erreur "The IP address configured for the host-only network is not within the allowed ranges" avec VirtualBox lors de l'utilisation de `192.168.56.110/24`, que cela signifie-t-il sur la configuration de l'hôte (pas de la VM) ?

**Q15.** Calcule combien de sous-réseaux `/27` peuvent être créés à partir d'un bloc `/24`.

---

## Section 5.B — Vagrant et types de réseaux (Q16-Q25)

**Q16.** Décris, sous forme de schéma ASCII simple, le chemin réseau suivi par un paquet allant de ta machine hôte vers une VM Vagrant configurée en `private_network` avec l'IP `192.168.56.110`.

**Q17.** Décris, sous forme de schéma ASCII, le chemin suivi par un paquet allant d'une VM Vagrant (réseau NAT) vers un serveur sur Internet (par exemple pour télécharger K3s).

**Q18.** Dans le projet, pourquoi la VM a-t-elle DEUX adresses IP distinctes (une via NAT, une via private_network) visibles avec `ip a` ?

**Q19.** Si tu désactivais complètement l'interface NAT d'une VM Vagrant (en ne gardant que le `private_network`), le script `server.sh` qui télécharge K3s via `curl https://get.k3s.io` fonctionnerait-il encore ? Justifie.

**Q20.** Pourquoi deux VMs définies dans des Vagrantfiles différents (dossiers `p1/` et `p2/` du projet) peuvent-elles toutes les deux utiliser l'adresse `192.168.56.110` sans conflit si elles ne tournent jamais simultanément ? Que se passerait-il si elles tournaient en même temps ?

**Q21.** Sous forme de schéma ASCII, représente l'architecture réseau complète de la Partie 1 du projet (2 VMs, leurs IPs, et la machine hôte).

**Q22.** Pourquoi le réseau `private_network` de VirtualBox est-il qualifié de "host-only" alors qu'il permet aussi aux VMs de communiquer entre elles (pas seulement avec l'hôte) ?

**Q23.** Quelle commande shell, exécutée sur la machine hôte (pas dans la VM), permet de vérifier que l'interface réseau host-only de VirtualBox existe bien sur le système ?

**Q24.** Si `vagrant up` échoue avec une erreur réseau liée à une plage d'IP non autorisée, quel fichier de configuration système (hors Vagrantfile) doit potentiellement être modifié sur l'hôte, et que doit-il contenir ?

**Q25.** Explique pourquoi, dans le contexte de ce projet, on choisit `private_network` plutôt que `public_network` pour le réseau du cluster K3s.

---

## Section 5.C — Détection d'erreurs de configuration (Q26-Q35)

**Q26.** Un étudiant configure ses deux VMs ainsi :
```
Serveur : 192.168.56.110/24
Worker  : 192.168.57.111/24
```
Le worker n'arrive jamais à joindre l'API du serveur sur le port 6443, alors que les deux VMs démarrent sans erreur. Diagnostique précisément le problème réseau.

**Q27.** Un étudiant a cette configuration réseau :
```
Serveur : 192.168.56.110, masque 255.255.255.0
Worker  : 192.168.56.111, masque 255.255.0.0
```
Les deux machines ont des masques différents sur le même sous-réseau apparent. Cela pose-t-il un problème de communication direct ? Justifie précisément.

**Q28.** Un évaluateur constate qu'il peut `ping` le serveur K3s depuis sa machine hôte, mais que `curl https://192.168.56.110:6443` échoue avec "Connection refused". Liste au moins 3 causes possibles distinctes.

**Q29.** Une étudiante a inversé les rôles : son worker.sh contient `SERVER_IP="192.168.56.111"` et `WORKER_IP="192.168.56.110"`, alors que son Vagrantfile attribue .110 au serveur et .111 au worker. Quel sera le symptôme observé lors du `vagrant up` ?

**Q30.** Un script worker tente de joindre l'API avec `curl -sk https://192.168.56.110:6444/healthz` (port 6444 au lieu de 6443). Quelle sera la conséquence exacte ?

**Q31.** Sur la VM serveur, `ss -tlnp | grep 6443` affiche :
```
LISTEN 0 4096 127.0.0.1:6443 0.0.0.0:*
```
Aucune ligne avec l'IP `192.168.56.110`. Diagnostique le problème et propose une piste de résolution liée au script d'installation K3s.

**Q32.** Une étudiante a ce schéma réseau :
```
Hôte           : 192.168.1.50/24  (réseau Wi-Fi domestique)
VM serveur     : 192.168.56.110/24 (private_network Vagrant)
```
Elle se demande pourquoi son téléphone connecté au même Wi-Fi que son ordinateur ne peut pas accéder à `192.168.56.110`. Explique-lui pourquoi c'est normal.

**Q33.** Un firewall sur la machine hôte bloque tout le trafic entrant sur les ports supérieurs à 1024 sauf exceptions explicites. Quel impact cela a-t-il potentiellement sur la capacité de la VM serveur à recevoir des connexions du worker sur le port 6443 ? (6443 > 1024)

**Q34.** Une configuration place le worker sur le réseau `192.168.56.0/25` (donc plage .0 à .127) avec l'IP `192.168.56.200`. Explique pourquoi cette IP est invalide pour ce sous-réseau précis.

**Q35.** Deux VMs sont configurées avec la même adresse MAC virtuelle par erreur de copier-coller dans une configuration VirtualBox avancée. Quel type de problème réseau cela peut-il provoquer, au-delà du simple conflit d'IP ?

---

## Section 5.D — DNS, DHCP, TCP/UDP (Q36-Q45)

**Q36.** Explique étape par étape ce qui se passerait si tu tapais `app1.com` directement dans la barre d'adresse d'un navigateur, sans avoir configuré ton fichier `/etc/hosts` ni de serveur DNS pour ce domaine, dans le contexte du projet IoT Partie 2.

**Q37.** Pourquoi la commande `curl -H "Host:app1.com" http://192.168.56.110` fonctionne-t-elle sans configuration DNS, alors qu'un vrai navigateur taper "app1.com" ne fonctionnerait pas sans entrée DNS ou fichier hosts ?

**Q38.** Si tu voulais réellement taper `http://app1.com` dans un navigateur et que cela fonctionne avec le cluster du projet, quelle modification minimale du fichier `/etc/hosts` de ta machine devrais-tu faire ?

**Q39.** Explique le rôle de CoreDNS à l'intérieur d'un cluster Kubernetes/K3s, et donne un exemple de nom DNS interne généré automatiquement pour un Service nommé `app-one` dans le namespace `default`.

**Q40.** Pourquoi DHCP n'a pas sa place dans la configuration réseau de la Partie 1 du projet, alors qu'il est omniprésent dans les réseaux domestiques classiques ?

**Q41.** Donne un exemple, hors contexte Kubernetes, où UDP serait préférable à TCP pour une application réseau, et explique pourquoi.

**Q42.** Pourquoi le protocole utilisé par l'API Kubernetes (HTTPS sur TCP) nécessite-t-il un établissement de connexion préalable (handshake), contrairement à un simple envoi UDP ?

**Q43.** Un étudiant affirme : "Le port 6443 de mon serveur K3s est ouvert car je peux le ping." Explique pourquoi cette affirmation est techniquement incorrecte.

**Q44.** Explique la différence entre une résolution DNS réussie mais un service inaccessible (le nom se résout en IP mais la connexion échoue), versus un échec de résolution DNS pur.

**Q45.** Dans le contexte de Traefik (Ingress Controller de K3s), explique pourquoi le routage par nom d'hôte HTTP (`Host` header) ne nécessite techniquement aucune configuration DNS spécifique côté serveur pour fonctionner avec `curl -H "Host:..."`.

---

## Section 5.E — Schémas et architecture réseau (Q46-Q50)

**Q46.** Dessine en ASCII l'architecture réseau complète de la Partie 3 du projet (K3d dans Docker), en représentant : la machine hôte, le moteur Docker, les conteneurs server/agent K3d, le load balancer K3d, et les ports exposés (8080, 8443).

**Q47.** Dessine en ASCII le chemin complet d'une requête HTTP dans la Partie 2 du projet, depuis `curl -H "Host:app2.com" http://192.168.56.110` jusqu'à l'un des 3 pods de l'application 2 (qui a 3 réplicas), en faisant apparaître l'Ingress, le Service et les pods.

**Q48.** Voici un schéma réseau incomplet de la Partie 1 du projet. Complète les éléments manquants (marqués `???`) :
```
...
**Q49.** Un schéma réseau montre une flèche allant du worker vers le serveur sur le port 6443, et une autre flèche allant du serveur vers le worker sans préciser de port. Sachant le fonctionnement de K3s (jonction puis communication continue), quel(s) port(s) et quelle direction de communication manquent probablement à ce schéma pour qu'il soit complet (kubelet, métriques, logs) ?

**Q50.** Dessine en ASCII la différence entre l'architecture réseau de la Partie 1 (2 VMs séparées avec Vagrant) et celle de la Partie 3 (cluster K3d dans des conteneurs Docker sur une seule machine), en mettant en évidence pourquoi la seconde nécessite moins de "sauts" réseau physiques/virtuels.

---

*Fin de la Partie 5. Passe à la Partie 6 (Kubernetes/K3s) avant de consulter le corrigé.*
# PARTIE 6 — KUBERNETES / K3S (50 exercices)

## Section 6.A — Architecture et composants (Q1-Q12)

**Q1.** Liste les 4 composants principaux du Control Plane Kubernetes et donne le rôle de chacun en une phrase précise.

**Q2.** Explique pourquoi on dit que kubectl ne "parle" jamais directement aux pods, mais toujours à l'API Server.

**Q3.** Pourquoi K3s n'installe-t-il pas etcd par défaut, et quelle alternative légère utilise-t-il à la place ?

**Q4.** Le kubelet tourne-t-il uniquement sur les workers, ou aussi potentiellement sur le nœud du Control Plane ? Justifie en lien avec le projet (Partie 1, une seule VM serveur qui fait aussi tourner des pods système).

**Q5.** Explique la différence de responsabilité entre le Scheduler et le Controller Manager.

**Q6.** Si l'API Server tombe en panne mais que les pods sont déjà en cours d'exécution sur les workers, ces pods s'arrêtent-ils immédiatement ? Justifie.

**Q7.** Pourquoi dit-on que kube-proxy est responsable de l'implémentation effective des Services Kubernetes au niveau réseau ?

**Q8.** Qu'est-ce qu'un "Container Runtime" dans l'architecture Kubernetes, et lequel est utilisé par défaut dans K3s ?

**Q9.** Explique pourquoi K3s regroupe la plupart de ses composants (API Server, Scheduler, kubelet...) en un seul binaire, contrairement à Kubernetes standard qui les distribue en plusieurs processus séparés.

**Q10.** Dans la Partie 1 du projet, le serveur K3s assure-t-il uniquement le rôle de Control Plane, ou peut-il aussi exécuter des pods applicatifs ? Justifie avec la commande `kubectl get nodes` et la colonne ROLES.

**Q11.** Pourquoi la commande `kubectl get nodes` doit-elle nécessairement réussir avant qu'un script de provisioning ne copie le `node-token`, dans la logique du projet ?

**Q12.** Explique pourquoi un Control Plane unique (sans haute disponibilité, comme dans ce projet) représente un point de défaillance unique (single point of failure), et pourquoi cela reste acceptable dans un contexte pédagogique.

---

## Section 6.B — Pods, Deployments, ReplicaSets (Q13-Q22)

**Q13.** Explique pourquoi un Pod ne doit jamais être créé directement dans un contexte de production, et quel objet de plus haut niveau on utilise à la place.

**Q14.** Un Deployment a `replicas: 3`. Un des trois pods plante. Décris précisément la séquence d'événements jusqu'au retour à l'état stable.

**Q15.** Pourquoi le ReplicaSet n'est-il presque jamais manipulé directement par un utilisateur, alors qu'il existe bel et bien comme objet Kubernetes intermédiaire entre Deployment et Pod ?

**Q16.** Analyse ce manifeste et identifie ce qui ne correspond PAS aux bonnes pratiques vues dans le projet (sans erreur de syntaxe, juste une pratique à interroger) :
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
**Q17.** Pourquoi l'image `nginx:latest` est-elle généralement déconseillée en faveur d'un tag explicite comme `nginx:alpine` ou une version précise, particulièrement dans un contexte GitOps (Partie 3) ?

**Q18.** Explique ce que signifie concrètement "Pod éphémère" en donnant un exemple précis avec des IPs (avant/après un crash de pod).

**Q19.** Un Deployment a `maxSurge: 1` et `maxUnavailable: 0` avec `replicas: 3`. Pendant une mise à jour, quel est le nombre maximal de pods simultanément en cours d'exécution (ancienne + nouvelle version confondues) ?

**Q20.** Avec la même configuration que Q19, combien de pods minimum sont garantis disponibles à tout instant pendant la mise à jour ?

**Q21.** Pourquoi un `livenessProbe` qui échoue provoque un redémarrage du conteneur, alors qu'un `readinessProbe` qui échoue ne fait que le retirer temporairement du Service, sans redémarrage ?

**Q22.** Dans la Partie 2 du projet, pourquoi l'application 2 nécessite-t-elle exactement 3 réplicas selon le sujet, et qu'est-ce que cela permet de démontrer en termes de répartition de charge ?

---

## Section 6.C — Services et réseau interne (Q23-Q32)

**Q23.** Explique pourquoi un Service Kubernetes reste indispensable même quand on utilise un Ingress par-dessus.

**Q24.** Analyse ce Service et indique précisément ce qu'il sélectionne :
```yaml
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  - port: 80
    targetPort: 8080
```
Si un pod porte uniquement le label `app: app-one` (sans `tier: frontend`), sera-t-il inclus dans ce Service ?

**Q25.** Dans le Service de la Q24, quelle est la différence entre `port: 80` et `targetPort: 8080` ? Sur quel port le client doit-il se connecter, et sur quel port le conteneur écoute-t-il réellement ?

**Q26.** Pourquoi un Service de type `ClusterIP` ne permet-il pas, par lui-même, d'accéder à l'application depuis l'extérieur du cluster (depuis ta machine hôte par exemple) ?

**Q27.** `kubectl get endpoints app-one` retourne une liste vide (`<none>`). Le Service existe et n'a pas été supprimé. Liste au moins 2 causes racines distinctes possibles.

**Q28.** Explique le mécanisme par lequel un Service "suit" automatiquement les pods qui correspondent à son selector, même quand ces pods sont recréés avec de nouvelles IPs après un crash.

**Q29.** Pourquoi le nom DNS interne complet d'un Service "app-one" dans le namespace "default" est-il `app-one.default.svc.cluster.local`, et que représente chaque partie de ce nom ?

**Q30.** Depuis un pod situé dans le namespace `dev`, peux-tu directement utiliser `curl http://app-one` pour joindre un Service nommé `app-one` situé dans le namespace `default`, sans rien préciser de plus ? Justifie.

**Q31.** Explique la commande `kubectl port-forward svc/wil-playground -n dev 8888:8888` : que représente chaque partie de cette commande (svc/, le nom, le namespace, les deux nombres) ?

**Q32.** Pourquoi `kubectl port-forward` n'est-il pas une solution adaptée pour exposer durablement une application en production, à la différence d'un Ingress ou d'un LoadBalancer ?

---

## Section 6.D — Ingress et Traefik (Q33-Q40)

**Q33.** Analyse ce manifeste Ingress et identifie ce qui empêchera certainement le routage par défaut de fonctionner comme prévu (indice : ordre des règles) :
```yaml
...
**Q34.** Pourquoi un objet Ingress créé sans aucun Ingress Controller actif dans le cluster n'a, en pratique, aucun effet sur le trafic réel ?

**Q35.** Explique la différence entre `pathType: Prefix` et `pathType: Exact` dans une règle Ingress, avec un exemple concret pour chacun.

**Q36.** Si deux Ingress Controllers différents (Traefik et Nginx) étaient installés simultanément dans le même cluster, quel rôle jouerait l'annotation `kubernetes.io/ingress.class` pour éviter les conflits ?

**Q37.** Pourquoi K3s installe-t-il Traefik par défaut, alors que Kubernetes standard (kubeadm) n'installe aucun Ingress Controller automatiquement ?

**Q38.** `curl -H "Host:app2.com" http://192.168.56.110` renvoie une erreur 404. Le pod app-two est `Running` et le Service existe. Liste 2 causes possibles distinctes liées spécifiquement à l'Ingress (pas au pod ni au Service).

**Q39.** Pourquoi est-il pédagogiquement intéressant que le sujet du projet demande explicitement "Le Ingress n'est pas montré ici, montre-le à l'évaluateur" plutôt que de l'inclure dans les captures d'écran fournies ?

**Q40.** Explique pourquoi la commande `kubectl get ingress -o wide` peut afficher une colonne ADDRESS vide même quand l'Ingress fonctionne correctement avec Traefik en mode K3s par défaut (sans LoadBalancer cloud).

---

## Section 6.E — Namespaces, Labels, Selectors (Q41-Q46)

**Q41.** Pourquoi créer deux namespaces distincts (`argocd` et `dev`) dans la Partie 3 du projet plutôt que de tout mettre dans `default` ?

**Q42.** `kubectl get pods` (sans option `-n`) sur quel namespace agit-il par défaut, et comment changer ce namespace par défaut de façon persistante pour ta session kubectl ?

**Q43.** Deux objets différents (un Service et un Deployment) peuvent-ils porter exactement le même label `app: app-one` sans provoquer de conflit ? Justifie en expliquant le rôle réel des labels.

**Q44.** Pourquoi `selector.matchLabels` d'un Deployment doit correspondre EXACTEMENT (et pas partiellement) aux labels présents dans `template.metadata.labels`, alors qu'un Service peut, lui, se contenter d'un sous-ensemble des labels d'un pod pour le sélectionner ?

**Q45.** Si tu supprimes le namespace `dev` avec `kubectl delete namespace dev`, que se passe-t-il pour TOUS les objets (Deployments, Pods, Services) qui s'y trouvaient ?

**Q46.** Explique pourquoi une Annotation et un Label ne sont pas interchangeables, en donnant un exemple où utiliser une Annotation à la place d'un Label casserait le fonctionnement attendu d'un Service.

---

## Section 6.F — K3s spécifique et installation (Q47-Q50)

**Q47.** Le `node-token` généré sur le serveur K3s est utilisé par le worker via la variable `K3S_TOKEN`. Explique pourquoi ce token seul, sans connaître également `K3S_URL`, ne suffirait pas à un worker pour rejoindre le cluster.

**Q48.** Pourquoi est-il recommandé, comme observé dans le débogage du projet, de fixer une version précise de K3s (`INSTALL_K3S_VERSION="v1.28.15+k3s1"`) plutôt que de laisser le script utiliser la version "stable" la plus récente par défaut ?

**Q49.** Explique la chaîne de causalité technique qui a conduit, dans le débogage réel du projet, à des "Slow SQL" de plusieurs dizaines de secondes dans les logs K3s, et le lien avec les ressources (RAM/CPU) allouées à la VM.

**Q50.** Pourquoi la commande `curl -sk https://SERVER_IP:6443/healthz` peut se comporter différemment (retourner "ok" en clair vs. retourner une erreur 401 Unauthorized) selon la version de K3s installée, et quelle adaptation de script cela impose-t-elle pour rester robuste face à ce changement ?

---

*Fin de la Partie 6. Passe à la Partie 7 (Debugging) avant de consulter le corrigé.*
# PARTIE 7 — DEBUGGING (50 scénarios)

> Pour chaque scénario : décris la démarche de diagnostic complète, étape par étape (quelles commandes tu lances, dans quel ordre, et ce que tu cherches dans chaque résultat), puis identifie la cause la plus probable et la solution.

---

**Scénario 7.1 — Worker absent du cluster**
Tu lances `vagrant up`. Les deux VMs démarrent sans erreur apparente. Mais sur le serveur, `kubectl get nodes` n'affiche qu'une seule ligne (le serveur lui-même), le worker n'apparaît jamais. Démarche de diagnostic ?

**Scénario 7.2 — Token absent**
Le script worker reste bloqué indéfiniment sur "Token not available yet..." jusqu'au timeout final. Démarche de diagnostic ?

**Scénario 7.3 — API totalement inaccessible**
Depuis ta machine hôte, `curl -sk https://192.168.56.110:6443/healthz` ne répond rien du tout après 10 secondes (ni erreur, ni contenu). Démarche de diagnostic ?

**Scénario 7.4 — Pod CrashLoopBackOff**
`kubectl get pods` affiche un pod en état `CrashLoopBackOff` avec `RESTARTS: 7`. Démarche de diagnostic complète ?

**Scénario 7.5 — Service inaccessible malgré pod Running**
Le pod est `Running` et `Ready: 1/1`, mais `curl` vers le Service échoue avec "Connection refused". Démarche de diagnostic ?

**Scénario 7.6 — Erreur Vagrant "VT-x is not available"**
`vagrant up` échoue immédiatement avec un message mentionnant VERR_VMX_NO_VMX. Démarche de diagnostic et résolution ?

**Scénario 7.7 — Erreur VirtualBox "VM already exists"**
`vagrant up` échoue avec "A VirtualBox machine with the name X already exists". Démarche de diagnostic et résolution ?

**Scénario 7.8 — Erreur Libvirt storage pool**
Avec le provider libvirt, `vagrant up` échoue avec une erreur de storage pool introuvable. Quelle commande système utiliserais-tu pour investiguer l'état des storage pools libvirt ?

**Scénario 7.9 — DNS interne K3s cassé**
Un pod ne peut pas résoudre `app-one.default.svc.cluster.local`, mais peut accéder directement à l'IP du Service (ClusterIP). Démarche de diagnostic centrée sur CoreDNS ?

**Scénario 7.10 — Ingress retourne 404 systématique**
Toutes les requêtes vers l'Ingress, même avec le bon en-tête Host, retournent 404. Le Service et les pods existent et sont sains. Démarche de diagnostic ?

**Scénario 7.11 — Worker "NotReady" en permanence**
Le worker apparaît dans `kubectl get nodes` mais reste bloqué en état `NotReady` indéfiniment. Démarche de diagnostic ?

**Scénario 7.12 — vagrant ssh refuse la connexion**
`vagrant ssh ankammerS` échoue avec un timeout de connexion alors que `vagrant status` indique `running`. Démarche de diagnostic ?

**Scénario 7.13 — Guest Additions version mismatch**
Tu observes dans les logs `vagrant up` : "Guest Additions Version: 7.0.2, VirtualBox Version: 7.2", suivi d'erreurs de montage de dossier partagé. Démarche de diagnostic et solutions possibles (au moins 2) ?

**Scénario 7.14 — /vagrant introuvable sur une seule des deux VMs**
Le dossier `/vagrant` fonctionne parfaitement sur le serveur mais retourne "No such file or directory" sur le worker, alors que le même Vagrantfile semble configurer les deux VMs de façon identique. Démarche de diagnostic ?

**Scénario 7.15 — kubectl se bloque indéfiniment sans erreur**
`kubectl get nodes` ne retourne ni résultat ni erreur, le terminal reste bloqué jusqu'à un Ctrl+C manuel. Démarche de diagnostic ?

**Scénario 7.16 — Token copié mais invalide**
Le worker récupère bien un fichier token (non vide), mais l'installation K3s agent échoue avec une erreur d'authentification. Démarche de diagnostic ?

**Scénario 7.17 — Argo CD reste "OutOfSync" en permanence**
Malgré un `syncPolicy.automated` correctement configuré, l'application Argo CD reste bloquée en état "OutOfSync". Démarche de diagnostic ?

**Scénario 7.18 — Argo CD "ComparisonError"**
L'interface Argo CD affiche "ComparisonError: failed to load target state" pour ton application. Démarche de diagnostic ?

**Scénario 7.19 — Image Docker introuvable**
Un pod reste en `ImagePullBackOff`. `kubectl describe pod` mentionne "manifest unknown". Démarche de diagnostic ?

**Scénario 7.20 — Pod Pending indéfiniment**
Un pod reste en état `Pending` sans jamais démarrer, sans message d'erreur visible avec `kubectl get pods` seul. Démarche de diagnostic ?

**Scénario 7.21 — Port déjà utilisé pour K3d**
`k3d cluster create` échoue avec "port is already allocated". Démarche de diagnostic et résolution ?

**Scénario 7.22 — Docker daemon inaccessible**
Toute commande `docker` échoue avec "permission denied" ou "cannot connect to the Docker daemon". Démarche de diagnostic ?

**Scénario 7.23 — Conflit d'adresses IP entre deux projets Vagrant**
Tu as deux Vagrantfiles différents (p1/ et p2/) qui utilisent tous les deux `192.168.56.110`. Les deux VMs tournent simultanément. Quel symptôme réseau observerais-tu, et comment le diagnostiquer ?

**Scénario 7.24 — Le worker n'arrive pas à pinger le serveur**
`ping 192.168.56.110` depuis le worker timeout systématiquement, alors que les deux VMs sont `running`. Démarche de diagnostic ?

**Scénario 7.25 — Script de provisioning silencieusement en échec**
Le `vagrant up` se termine "avec succès" en apparence (pas de message d'erreur rouge), mais en se connectant à la VM, K3s n'est pas installé du tout. Démarche de diagnostic ?

**Scénario 7.26 — SQL lentes dans les logs K3s**
Les logs K3s (`/var/log/k3s.log`) affichent de nombreuses lignes "Slow SQL" avec des durées de 20 à 80 secondes. Démarche de diagnostic et lien avec les ressources de la VM ?

**Scénario 7.27 — Service avec Endpoints vides**
`kubectl get endpoints mon-service` retourne `<none>`. Démarche de diagnostic complète (au moins 3 pistes à vérifier) ?

**Scénario 7.28 — Application Argo CD jamais créée**
`kubectl apply -f argocd-app.yaml` ne retourne aucune erreur, mais `kubectl get application -n argocd` ne montre rien. Démarche de diagnostic ?

**Scénario 7.29 — Mise à jour Git non détectée par Argo CD**
Tu modifies `deployment.yaml`, tu push sur GitHub, mais après 10 minutes l'application Argo CD reste sur l'ancienne version. Démarche de diagnostic ?

**Scénario 7.30 — Erreur de certificat TLS K3s**
Le worker échoue avec une erreur mentionnant "x509: certificate is valid for X, not Y" lors de la tentative de jonction au serveur. Démarche de diagnostic ?

**Scénario 7.31 — VM qui boot indéfiniment ("Waiting for machine to boot")**
`vagrant up` reste bloqué plusieurs minutes sur "Waiting for machine to boot" sans jamais aboutir. Démarche de diagnostic ?

**Scénario 7.32 — kubectl pointe vers le mauvais cluster**
Après avoir créé un cluster K3d, `kubectl get nodes` affiche les nœuds d'un ANCIEN cluster K3d que tu pensais avoir supprimé. Démarche de diagnostic ?

**Scénario 7.33 — rc-service introuvable**
Un script de provisioning utilise `rc-service k3s status` mais échoue avec "command not found" sur la VM. Démarche de diagnostic ?

**Scénario 7.34 — Apk update échoue avec timeout réseau**
`apk update` dans le script de provisioning échoue avec un timeout de connexion vers les mirrors Alpine. Démarche de diagnostic (penser au réseau NAT de la VM) ?

**Scénario 7.35 — GitLab local inaccessible après installation Helm**
Après `helm install gitlab gitlab/gitlab`, tous les pods du namespace `gitlab` restent `Pending` pendant plus de 20 minutes. Démarche de diagnostic ?

**Scénario 7.36 — Argo CD : mot de passe admin refusé**
Tu récupères le secret `argocd-initial-admin-secret`, mais la connexion à l'interface web échoue avec "invalid username or password". Démarche de diagnostic ?

**Scénario 7.37 — Ingress fonctionne pour app1.com mais pas app2.com**
La règle Ingress pour `app1.com` route correctement, mais celle pour `app2.com` retourne systématiquement la réponse de l'application par défaut (app3). Démarche de diagnostic ?

**Scénario 7.38 — vagrant destroy ne supprime pas vraiment la VM**
Après `vagrant destroy -f`, la VM apparaît toujours dans `vboxmanage list vms`. Démarche de diagnostic ?

**Scénario 7.39 — Le serveur K3s consomme 100% de CPU en permanence**
`top` dans la VM serveur montre le processus k3s à 100% de CPU constant, même sans charge applicative. Démarche de diagnostic ?

**Scénario 7.40 — Deux pods avec le même nom apparente mais labels différents**
Tu observes deux Deployments différents qui créent chacun des pods, mais un Service censé cibler un seul Deployment reçoit du trafic destiné à être réparti, et inclut des pods inattendus. Démarche de diagnostic centrée sur les labels ?

**Scénario 7.41 — Token HTTP non accessible entre VMs (solution serveur HTTP)**
Tu utilises une solution alternative au dossier partagé : le serveur expose le token via un serveur HTTP Python sur le port 9999. Le worker ne parvient jamais à le récupérer (`curl` échoue avec "Connection refused"). Démarche de diagnostic ?

**Scénario 7.42 — Le cluster K3d perd ses données après un redémarrage de la machine hôte**
Après avoir éteint puis rallumé ta machine hôte, `k3d cluster list` ne montre plus aucun cluster, alors qu'il existait avant l'extinction. Démarche de diagnostic ?

**Scénario 7.43 — kubectl logs ne montre rien**
`kubectl logs mon-pod` retourne une sortie vide, alors que le pod est `Running` depuis 10 minutes. Démarche de diagnostic ?

**Scénario 7.44 — Health check (readinessProbe) qui échoue en boucle**
Un pod reste `Running` mais jamais `Ready` (`0/1`), et n'est jamais inclus dans le Service correspondant. Démarche de diagnostic centrée sur les probes ?

**Scénario 7.45 — Plusieurs versions de K3s en conflit**
Après une réinstallation manuelle de K3s sur le serveur, `k3s --version` affiche une version différente de celle utilisée pour générer le `node-token` actuel. Quel symptôme cela peut-il provoquer côté worker, et comment diagnostiquer cette incohérence ?

**Scénario 7.46 — VirtualBox indique 0 CPU disponible pour la VM**
La VM refuse de démarrer avec un message lié à l'absence de CPU disponible alors que la machine hôte a clairement plusieurs cœurs. Démarche de diagnostic (penser à la sur-allocation cumulée des VMs déjà actives) ?

**Scénario 7.47 — La VM atteint 100% d'utilisation disque pendant le provisioning**
Le script de provisioning échoue en cours d'exécution avec une erreur "No space left on device". Démarche de diagnostic ?

**Scénario 7.48 — Argo CD synchronise mais l'application ne se met pas à jour visuellement**
Argo CD affiche "Synced" et "Healthy", mais `curl http://localhost:8888/` continue de retourner l'ancienne version. Démarche de diagnostic (penser à `imagePullPolicy` et au cache local de l'image) ?

**Scénario 7.49 — Conflit entre service système et provisioning Vagrant au redémarrage**
Après un `vagrant reload` (sans `--provision`), K3s ne redémarre pas automatiquement alors qu'il était actif avant l'arrêt de la VM. Démarche de diagnostic (penser à la configuration OpenRC du service) ?

**Scénario 7.50 — Bonus GitLab : pipeline CI/CD GitLab Runner introuvable**
Tu essaies d'utiliser GitLab CI dans le cadre du bonus, mais aucun job ne démarre, restant indéfiniment en "pending", avec le message "This job is stuck because the project doesn't have any runners online". Démarche de diagnostic et lien avec la configuration Helm utilisée à l'installation ?

---

*Fin de la Partie 7. Passe à la Partie 8 (Oral blanc complet) avant de consulter le corrigé final.*
# PARTIE 8 — ORAL BLANC 42 (100 questions)

> Simulation de soutenance réelle. Les questions mélangent volontairement tous les niveaux et tous les sujets, dans le désordre, comme le ferait un évaluateur en conditions réelles. Certaines questions contiennent des pièges (affirmations fausses glissées dans la question, fausses évidences, confusions volontaires entre deux notions proches). Repère-les.

---

**Q1.** Présente-moi en 2 minutes l'architecture globale de ton projet, sans entrer dans les détails de code.

**Q2.** Tu m'as dit que K3s utilise etcd par défaut. Confirme-moi ce point. *(Piège : vérifie l'affirmation avant de répondre.)*

**Q3.** Pourquoi as-tu choisi Alpine Linux plutôt qu'Ubuntu pour tes VMs ?

**Q4.** Montre-moi ton Vagrantfile de la Partie 1 et explique-moi la ligne qui définit le réseau.

**Q5.** Le `private_network` de Vagrant donne-t-il un accès Internet à la VM ? Justifie.

**Q6.** Quelle est la commande pour voir tous les pods de tous les namespaces en une seule fois ?

**Q7.** Tu utilises kubectl pour gérer ton cluster. kubectl est-il un composant qui tourne en permanence sur le serveur, comme un service ? *(Piège : confusion entre client et composant serveur.)*

**Q8.** Que se passe-t-il exactement si tu supprimes un Pod géré par un Deployment avec `kubectl delete pod` ?

**Q9.** Explique-moi la différence entre `vagrant halt` et `vagrant destroy`.

**Q10.** Ton Service est de type NodePort. Pourquoi as-tu fait ce choix plutôt que ClusterIP pour la Partie 2 ? *(Piège : vérifie ce que tu as réellement utilisé.)*

**Q11.** Le `node-token` est-il identique pour tous les clusters K3s du monde, ou unique à chaque cluster ?

**Q12.** Pourquoi le sujet impose-t-il 3 réplicas pour l'application 2 et pas les autres ?

**Q13.** Que fait exactement la commande `kubectl get nodes -o wide` que `kubectl get nodes` seul ne montre pas ?

**Q14.** Ton script bash utilise `#!/bin/bash`. Pourquoi avoir choisi bash plutôt que sh pour du provisioning Alpine ? *(Piège : vérifie quel shebang tu utilises réellement dans tes scripts.)*

**Q15.** Explique la différence entre `[ ]` et `[[ ]]` en shell, et pourquoi Alpine BusyBox ne supporte pas systématiquement la seconde forme.

**Q16.** Le Control Plane et le Worker peuvent-ils être la même machine physique/virtuelle ? Justifie avec ton propre projet.

**Q17.** Pourquoi `set -e` est-il utile dans un script de provisioning, et donne-moi un cas où il aurait pu t'éviter un bug silencieux pendant ton développement.

**Q18.** Ton Ingress utilise l'annotation `nginx.ingress.kubernetes.io/rewrite-target`. Explique-moi son rôle. *(Piège : cette annotation est spécifique à Nginx Ingress, pas Traefik — vérifie ce que tu utilises réellement.)*

**Q19.** Que se passe-t-il si deux Deployments différents utilisent EXACTEMENT le même selector de labels ?

**Q20.** Explique-moi la différence entre TCP et UDP avec un exemple concret tiré de ton projet.

**Q21.** Le masque `/24` permet combien d'adresses hôtes utilisables ?

**Q22.** Pourquoi le worker doit-il attendre que le token existe avant de continuer son script ? Que se passerait-il sans cette attente ?

**Q23.** As-tu utilisé Docker Swarm dans ton projet ? *(Piège : Docker Swarm n'a jamais été demandé ni pertinent ici — vérifie que tu ne confonds pas avec K3s/K3d.)*

**Q24.** Montre-moi comment tu vérifies, en une seule commande, qu'un Service a bien des pods associés.

**Q25.** Pourquoi utilises-tu `curl -H "Host:app1.com"` plutôt que de configurer un vrai nom de domaine ?

**Q26.** Quelle est la différence fondamentale entre K3s et K3d ?

**Q27.** Ton Argo CD est configuré en mode manuel (pas automatique). Comment le sujet exige-t-il que la synchronisation se déclenche ? *(Piège : vérifie ta configuration syncPolicy réelle vs ce que tu affirmes.)*

**Q28.** Explique-moi ce qu'est un ReplicaSet et sa relation avec un Deployment.

**Q29.** Pourquoi un masque réseau est-il indissociable d'une adresse IP pour en déterminer la portée ?

**Q30.** As-tu rencontré des problèmes avec les Guest Additions VirtualBox pendant ton développement ? Explique le problème technique précis.

**Q31.** Ta VM serveur a 512 Mo de RAM allouée. Est-ce suffisant pour K3s v1.35 ? *(Piège : tu as probablement dû augmenter la RAM en cours de débogage — sois honnête sur ce que tu utilises réellement.)*

**Q32.** Quelle commande shell permet de vérifier qu'un point de montage existe réellement, au-delà de simplement vérifier que le dossier existe ?

**Q33.** Explique-moi pourquoi un curl vers `/healthz` peut retourner 401 sur les versions récentes de K3s, et comment tu as adapté ton script en conséquence.

**Q34.** Le `kubeconfig` contient-il le mot de passe root du serveur ? *(Piège : confusion entre certificats client TLS et mot de passe — clarifie la vraie nature des credentials.)*

**Q35.** Donne-moi la définition exacte d'une socket réseau.

**Q36.** Pourquoi ton script attend-il 40 secondes après l'installation de K3s avant de tester l'API ?

**Q37.** Ton cluster utilise Calico comme CNI. Peux-tu m'expliquer pourquoi tu as fait ce choix ? *(Piège : K3s utilise Flannel par défaut, pas Calico — vérifie ce qui est réellement utilisé.)*

**Q38.** Quelle est la différence entre `apk add` et `apt install` au-delà du nom de la commande ?

**Q39.** Explique-moi le rôle exact de `INSTALL_K3S_EXEC` dans le script d'installation K3s.

**Q40.** Un Pod a le statut `Completed`. Est-ce un problème ? Justifie selon le contexte.

**Q41.** Pourquoi dois-tu copier le `node-token` quelque part accessible par le worker, plutôt que de le générer indépendamment sur chaque machine ?

**Q42.** Quelle est la signification exacte du `?` dans `echo $?` ?

**Q43.** Ton Ingress route vers le Service par IP directe du pod plutôt que par nom de Service. Est-ce une bonne pratique ? *(Piège : un Ingress backend pointe normalement vers un Service, pas une IP de pod — vérifie ta config réelle.)*

**Q44.** Explique-moi la chaîne complète depuis `git push` jusqu'à la mise à jour effective du pod dans le cluster, dans ta Partie 3.

**Q45.** Pourquoi `prune: true` dans la syncPolicy d'Argo CD peut être dangereux si mal compris ?

**Q46.** Le sujet impose-t-il que ton Vagrantfile utilise `config.vm.box` identique pour toutes tes VMs ? Justifie avec une référence au sujet.

**Q47.** Quelle est la différence entre `kubectl apply` et `kubectl create` ?

**Q48.** Pourquoi recommande-t-on de toujours guillemeter les variables shell dans les tests `[ ]` ?

**Q49.** Le Control Plane stocke-t-il les logs applicatifs de tous les pods de façon centralisée par défaut dans Kubernetes/K3s natif ? *(Piège : ce n'est pas le cas nativement, sans solution de logging centralisé déployée séparément.)*

**Q50.** Explique-moi pourquoi tu as dû gérer un montage manuel de `/vagrant` à un moment de ton développement.

---

**Q51.** Le réseau `192.168.56.0/24` et `192.168.57.0/24` peuvent-ils communiquer directement sans gateway ? Justifie.

**Q52.** Pourquoi `kubectl get pods` sans namespace spécifié ne montre-t-il pas les pods système comme CoreDNS ou Traefik ?

**Q53.** As-tu utilisé un PersistentVolume dans ton projet ? Si non, pourquoi n'était-ce pas nécessaire pour les applications de la Partie 2 ?

**Q54.** Quelle différence fais-tu entre `vagrant provision` et le fait de relancer manuellement un script via SSH ?

**Q55.** Explique pourquoi `curl -sk` ignore la vérification du certificat TLS, et le risque que cela représenterait en production.

**Q56.** Ton serveur K3s utilise le port 8080 pour l'API. *(Piège : le port standard est 6443, pas 8080 — vérifie l'affirmation.)*

**Q57.** Pourquoi le sujet demande-t-il explicitement de désactiver `box_check_update` selon toi (même si ce n'est pas une obligation stricte du sujet) ?

**Q58.** Quelle est la différence entre `INSTALL_K3S_VERSION` et `INSTALL_K3S_CHANNEL` dans le script d'installation K3s ?

**Q59.** Explique-moi le principe du GitOps en une phrase, puis détaille avec ton propre exemple de la Partie 3.

**Q60.** As-tu donné les droits d'administrateur cluster complet (cluster-admin) à Argo CD par défaut à l'installation ? Quelle en est l'implication de sécurité, même si ce n'est pas un sujet central du projet ?

**Q61.** Pourquoi un `ping` qui échoue n'implique-t-il pas forcément que TOUTE communication réseau échoue entre deux machines ?

**Q62.** Le sujet exige-t-il que tu utilises exactement Docker Hub, ou un autre registre est-il acceptable pour les images de la Partie 3 ?

**Q63.** Explique-moi la commande complète que tu utilises pour forcer une synchronisation manuelle Argo CD en ligne de commande.

**Q64.** Une boucle `until` et une boucle `while` avec une condition inversée sont-elles toujours strictement équivalentes en pratique ? Donne un exemple où l'une serait plus lisible que l'autre.

**Q65.** Pourquoi avoir choisi (ou pas) d'exposer le port 9999 via un mini serveur HTTP Python comme alternative au dossier partagé Vagrant ? Quels sont les compromis de cette approche ?

**Q66.** Le sujet impose-t-il une limite stricte de RAM par VM (512 Mo exactement) ou une recommandation ? Justifie avec le texte du sujet.

**Q67.** Que se passe-t-il concrètement si tu oublies `--write-kubeconfig-mode=644` à l'installation du serveur K3s ?

**Q68.** As-tu mis en place un volume persistant pour la base SQLite de K3s afin de survivre à une suppression de VM ? Pourquoi est-ce pertinent ou non dans ce projet ?

**Q69.** Explique pourquoi la commande `kubectl rollout restart deployment/mon-app` peut être utile pour forcer un redémarrage propre sans changer la configuration.

**Q70.** Le bonus GitLab impose-t-il d'utiliser exclusivement Helm, ou d'autres méthodes d'installation sont-elles envisageables ? Quel est l'avantage de Helm ici ?

**Q71.** Quelle est la différence entre `kubectl logs pod` et `kubectl logs pod --previous` ?

**Q72.** Ton Deployment de l'application 2 a `replicas: 3`, mais tu observes parfois seulement 2 pods Running. Que devrais-tu vérifier immédiatement ?

**Q73.** Explique pourquoi un `node-token` copié via dossier partagé pose un problème spécifique si les Guest Additions ne sont pas synchronisées avec la version de VirtualBox.

**Q74.** Le `K3S_URL` doit-il pointer vers l'IP NAT de la VM serveur ou vers son IP private_network ? Justifie avec le contexte du projet.

**Q75.** As-tu vérifié que ton Ingress fonctionne aussi correctement après un redémarrage complet des VMs (`vagrant reload`), pas seulement au premier `vagrant up` ?

---

**Q76.** Pourquoi dit-on que kubectl est un client "stateless" qui ne conserve aucune information entre deux appels indépendants ?

**Q77.** As-tu eu besoin de configurer manuellement un Network Policy Kubernetes dans ton projet ? Si non, pourquoi le sujet ne l'exige-t-il pas explicitement ?

**Q78.** Explique en quoi consiste exactement le "rolling update" que Kubernetes effectue automatiquement quand tu changes l'image d'un Deployment.

**Q79.** Pourquoi `kubectl get pods -n kube-system` montre-t-il des pods que toi-même n'as jamais créés explicitement ?

**Q80.** Sur quel port le Service de l'application 2 écoute-t-il dans ta configuration, et est-ce le même port que celui sur lequel le conteneur écoute réellement ?

**Q81.** Le sujet impose-t-il un nombre maximum de CPU par VM ? Que recommande-t-il précisément ?

**Q82.** Explique-moi pourquoi avoir une seule VM en Partie 2 (contrairement aux 2 VMs en Partie 1) simplifie certains aspects du déploiement, mais en complique potentiellement d'autres.

**Q83.** As-tu rencontré le bug du pipe cassé (`INSTALL_K3S_EXEC="..." \` suivi de `sh -` sur une nouvelle ligne) ? Explique précisément pourquoi cette erreur de syntaxe shell est si insidieuse à détecter.

**Q84.** Quelle est la différence entre executer un script avec `sh script.sh` et avec `./script.sh` (en supposant les permissions adéquates) ?

**Q85.** Le Control Plane K3s expose-t-il une interface web d'administration native, comme le Kubernetes Dashboard ? Si non, comment visualises-tu l'état de ton cluster ?

**Q86.** Explique pourquoi la commande `kubectl get pods --watch` est différente de relancer `kubectl get pods` en boucle avec un script.

**Q87.** As-tu testé le comportement de ton Ingress quand AUCUN en-tête Host personnalisé n'est fourni (cas de l'application 3 par défaut) ? Détaille comment cette règle par défaut doit être positionnée dans le manifeste.

**Q88.** Pourquoi le sujet précise-t-il que les captures d'écran d'exemple utilisent des interfaces réseau `eth0`/`eth1`, alors qu'un système Linux moderne utilise plutôt des noms comme `enp0s8` ?

**Q89.** Explique la différence entre une image Docker "multi-stage build" et une image classique en un seul stage (même si ton projet n'en a pas forcément besoin).

**Q90.** As-tu utilisé `kubectl exec` pour déboguer un pod en cours d'exécution à un moment de ton développement ? Donne un exemple concret de commande que tu aurais pu utiliser.

---

**Q91.** Pourquoi dit-on que ton cluster K3s de la Partie 1 n'a pas de haute disponibilité (HA) ?

**Q92.** Explique pourquoi changer la version de K3s en cours de route (sans réinstaller le worker en même temps) peut créer une incompatibilité.

**Q93.** Le sujet exige-t-il que les deux applications du bonus (Argo CD + GitLab) tournent sur la MÊME instance K3d que la Partie 3, ou peuvent-elles être totalement séparées ? Justifie avec le texte du sujet.

**Q94.** Quelle est la signification exacte de l'abréviation "CIDR" ?

**Q95.** As-tu vérifié, lors de ta soutenance, que tu sais reproduire l'intégralité du processus depuis zéro (VM détruites) en un temps raisonnable devant l'évaluateur ?

**Q96.** Explique pourquoi un évaluateur pourrait te demander de casser volontairement quelque chose (par exemple supprimer un pod) pour vérifier que ton cluster se rétablit automatiquement.

**Q97.** Le sujet impose-t-il explicitement le nom exact des namespaces (`argocd` et `dev`), ou est-ce une convention que tu as suivie ? Vérifie la formulation exacte du sujet.

**Q98.** Pourquoi peut-on dire que la Partie 1 du projet (Vagrant + K3s manuel) est conceptuellement plus proche d'un déploiement "on-premise" traditionnel, alors que la Partie 3 (K3d + Argo CD) se rapproche davantage des pratiques DevOps modernes ?

**Q99.** As-tu une opinion sur ce que tu changerais dans ton architecture si tu devais refaire ce projet avec plus de temps (haute disponibilité, sécurité, monitoring) ?

**Q100.** Question finale ouverte : explique-moi, dans tes propres mots et sans relire aucune note, ce que tu as concrètement appris de ce projet sur la façon dont les entreprises modernes gèrent leurs applications en production.

---

*Fin de la Partie 8 et de tous les exercices. Le corrigé complet de l'intégralité des Parties 1 à 8 suit ci-dessous.*
---
---
